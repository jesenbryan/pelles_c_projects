...
static HMENU buildMainMenu(void)
{
    ...
    // Below Hide Inactive Layer, same menu -- toggles canvas.c's real-time
    // FPS counter (top-right, above the Probe 2 Settle button; see
    // showFpsOverlay's own comment). MF_CHECKED here to match
    // showFpsOverlay's own TRUE-by-default (a real report of the
    // simulation "sometimes isn't as smooth" is exactly why this exists,
    // so it should already be visible on the first frame, not require an
    ...
    // handler enables them on entering Simulation and greys them again on
    // leaving. Appended at the END of View, so the fixed GetSubMenu(hMenuBar, 2)
    // lookup the other View handlers rely on is unaffected.
    ...
    // Matches showFpsOverlay's own default (TRUE, canvas.c) -- same
    // "already set via AppendMenu's own MF_CHECKED flag, kept explicit
    // anyway" reasoning as ID_TOGGLE_HIDE_INACTIVE's own call just above.
    CheckMenuItem(hViewMenu, ID_TOGGLE_SHOW_FPS, MF_BYCOMMAND | MF_CHECKED);
