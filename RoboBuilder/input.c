...
// Parses the Weight box's text. Accepts '.' or ',' as the decimal
// separator (a German-locale habit, "1,5"), and returns FALSE -- leaving
// the robot's current weight untouched -- for empty or non-numeric text,
// instead of wcstod's silent 0.0 (which is how a half-typed or cleared box
// used to zero a robot's weight).
static BOOL parseWeightText(const wchar_t* text, float* out)
{
    wchar_t buf[64];
    int n = 0;
    for (const wchar_t* p = text; *p && n < 63; p++)
        buf[n++] = (*p == L',') ? L'.' : *p;
    buf[n] = 0;

    wchar_t* end = NULL;
    double v = wcstod(buf, &end);
    if (end == buf) return FALSE;          // nothing numeric at all
...
// The active robot kind's actualWeight (the Weight box's backing value).
static float* activeKindActualWeightPtr(AppState* app)
{
    switch (app->robotScene.activeKind)
    {
        case ROBOT_KIND_ROCKY: return &app->robotScene.rocky.actualWeight;
        case ROBOT_KIND_STILO: return &app->robotScene.stilo.actualWeight;
        case ROBOT_KIND_SEMNI:
        default:               return &app->robotScene.robot.actualWeight;
    }
}

// Rewrites the Weight box from the active kind's weight, always in the same
// "1.00" two-decimal format (it used to start as "1.0" and only switch to
// "1.00" after a robot switch). Typing is only ever read back on focus loss
// (commitWeightEdit), never on EN_CHANGE, so writing the text here can't be
// mistaken for user input.
static void refreshWeightEdit(AppState* app)
{
    if (!app->ui.hActualWeightEdit) return;
    wchar_t buf[32];
    formatNonNegativeFloat2dp(*activeKindActualWeightPtr(app), buf);
    SetWindowText(app->ui.hActualWeightEdit, buf);
}

// Confirms whatever is typed in the Weight box: a valid number becomes the
// active kind's weight; anything else (empty, half-typed, letters) is
// discarded. Either way the box is then reformatted from the stored value,
// so it always shows what's actually in effect. Called when the box loses
// focus (Enter, or clicking anything else -- see weightEditSubclassProc and
// the ID_ACTUAL_WEIGHT_EDIT EN_KILLFOCUS handler) and by File > Save.
void commitWeightEdit(AppState* app)
{
    if (!app->ui.hActualWeightEdit) return;
    wchar_t buf[64];
    GetWindowText(app->ui.hActualWeightEdit, buf, 64);
    float value;
    if (parseWeightText(buf, &value))
        *activeKindActualWeightPtr(app) = value;
    refreshWeightEdit(app);
}
...
// World-space midpoint of one of Rocky's 4 rectangle edges -- same
// bodyX/bodyY +/- bodyHalfWidth/bodyHalfHeight construction, then
// rotatePoint by the body's own angle, that renderer.c's drawRocky uses
// to actually draw each edge's handle (leftMid/rightMid/bottomMid/
// topMid). Exists so rockyMassCenterWins (below) has a real world-space
// point to compare distances against when the mass-center dot's range
// happens to pass near an edge, not just near Body/Knee/Foot/Shin.
static PointF rockyEdgeMidWorld(Rocky r, int edge, PointF center)
{
    PointF local;
    switch (edge)
    {
        case ROCKY_EDGE_LEFT:   local = (PointF){ r.bodyX - r.bodyHalfWidth, r.bodyY }; break;
        case ROCKY_EDGE_RIGHT:  local = (PointF){ r.bodyX + r.bodyHalfWidth, r.bodyY }; break;
        case ROCKY_EDGE_BOTTOM: local = (PointF){ r.bodyX, r.bodyY - r.bodyHalfHeight }; break;
        case ROCKY_EDGE_TOP:    local = (PointF){ r.bodyX, r.bodyY + r.bodyHalfHeight }; break;
        default:                local = center; break;
    }
    return rotatePoint(local, center, r.angle);
}

static float rockyDistSq(PointF a, PointF b)
{
    float dx = a.x - b.x, dy = a.y - b.y;
    return dx * dx + dy * dy;
}

// Whether the Body/Leg Weight mass-center dot should win a hit-test tie
// against whichever OTHER Rocky handle the mouse is ALSO currently within
// range of. The dot's own reachable range (rect-centroid<->leg-centroid,
// computeRockyMassCenterEndpointsWorld) can drift close to the knee, foot,
// a shin arc handle, or an edge midpoint depending on the current Body/Leg
// Weight split and leg pose -- not just the Body handle's documented
// exact-coincidence case (legWeight ~ 0). Unconditionally giving the dot
// priority over everything (the previous behavior) meant it could quietly
// steal clicks/hovers meant for whatever other handle it happened to be
// sitting near, which is exactly the "doesn't work well near another
// handle" symptom. This instead picks whichever candidate the mouse is
// actually closer to, with an exact tie (the documented Body-coincidence
// case) still going to the mass-center dot, same as before.
static BOOL rockyMassCenterWins(PointF mouse, PointF massCenter, PointF rockyCenter,
                                 int rockyEdgeHit, Rocky rocky,
                                 PointF kneeWorld, PointF footWorld,
                                 PointF shin1World, PointF shin2World)
{
    PointF otherPoint = rockyCenter;
    BOOL haveOther = TRUE;

    if (isNear(mouse, rockyCenter, HIP_HANDLE_RADIUS))
        otherPoint = rockyCenter;
    else if (rockyEdgeHit != ROCKY_EDGE_NONE)
        otherPoint = rockyEdgeMidWorld(rocky, rockyEdgeHit, rockyCenter);
    else if (isNear(mouse, kneeWorld, KNEE_HANDLE_RADIUS))
        otherPoint = kneeWorld;
    else if (isNear(mouse, footWorld, FOOT_HANDLE_RADIUS))
        otherPoint = footWorld;
    else if (isNear(mouse, shin1World, ROCKY_SHIN_HANDLE_RADIUS))
        otherPoint = shin1World;
    else if (isNear(mouse, shin2World, ROCKY_SHIN_HANDLE_RADIUS))
        otherPoint = shin2World;
    else
        haveOther = FALSE;

    if (!haveOther)
        return TRUE; // nothing else in range -- mass center wins by default

    return rockyDistSq(mouse, massCenter) <= rockyDistSq(mouse, otherPoint); // tie -> mass center
}

// Whether a rectangle-edge hit (Width/Height resize) should actually win
// against Knee/Foot/Shin1/Shin2 when the mouse is ALSO within range of one
// of those. hitTestRockyEdge deliberately extends a bit past each edge's
// own span (its own "slack" margin) so a drag started slightly past the
// edge's true endpoint still grabs it -- but that same slack means the
// edge's hit region reaches into a CORNER, where the knee (or foot/shin)
// handle can legitimately be sitting too (reported: a knee parked right in
// a corner became impossible to grab at all, since the edge unconditionally
// won every time). Same "whichever the mouse is actually closer to wins"
// principle as rockyMassCenterWins above, just for this other pairing.
static BOOL rockyEdgeWins(PointF mouse, PointF edgeMid, PointF kneeWorld, PointF footWorld, PointF shin1World, PointF shin2World)
{
    float edgeDistSq = rockyDistSq(mouse, edgeMid);

    if (isNear(mouse, kneeWorld, KNEE_HANDLE_RADIUS) && rockyDistSq(mouse, kneeWorld) < edgeDistSq)
        return FALSE;
    if (isNear(mouse, footWorld, FOOT_HANDLE_RADIUS) && rockyDistSq(mouse, footWorld) < edgeDistSq)
        return FALSE;
    if (isNear(mouse, shin1World, ROCKY_SHIN_HANDLE_RADIUS) && rockyDistSq(mouse, shin1World) < edgeDistSq)
        return FALSE;
    if (isNear(mouse, shin2World, ROCKY_SHIN_HANDLE_RADIUS) && rockyDistSq(mouse, shin2World) < edgeDistSq)
        return FALSE;
...
// The robot panel's background: a click on empty panel space moves focus to
// the main window, so it also confirms a value being typed in the Weight
// box (EN_KILLFOCUS -> commitWeightEdit), same as clicking the canvas.
static LRESULT CALLBACK semniPanelWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
static void ensureSemniPanelClassRegistered(void)
{
    ...
    WNDCLASS wc = {0};
    wc.lpfnWndProc = semniPanelWndProc;
...
static LRESULT CALLBACK weightEditSubclassProc(HWND hEdit, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    {
        // Esc cancels the edit: drop the typed text, then leave the box.
        // commitWeightEdit on the focus loss re-reads the (restored) text.
        refreshWeightEdit(&app);
    ...
    if (msg == WM_CHAR && (wParam == VK_RETURN || wParam == VK_ESCAPE))
        return 0;   // no beep for the Enter/Esc handled on WM_KEYDOWN

    if (msg == WM_KEYDOWN && wParam == VK_RETURN)
    {
        // Enter confirms: moving focus away fires EN_KILLFOCUS, which
        // commits the value (commitWeightEdit).
        SetFocus((HWND)dwRefData);
...
LRESULT handleInput(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, AppState* app)
{
    switch (msg)
    {
        ...
        case WM_LBUTTONDOWN:
        {
            // Clicking the canvas confirms a value being typed in the
            // Weight box (focus leaves it -> EN_KILLFOCUS -> commitWeightEdit).
            if (GetFocus() != hwnd)
                SetFocus(hwnd);
            ...
            if (app->robotScene.activeKind == ROBOT_KIND_ROCKY)
            {
                ...
                // Mass-center dot -- checked first, but only actually wins
                // when rockyMassCenterWins agrees it's the CLOSER of the
                // two (see that function's own comment): the dot's own
                // reachable range runs along the rect-centroid<->leg-
                // centroid segment (renderer.c's
                ...
                // of each other, and the tie there deliberately still
                // goes to the dot. But that same reachable range can also
                // drift close to the knee, foot, a shin arc handle, or an
                // edge midpoint depending on pose -- unconditionally
                // giving the dot priority in ALL of those cases too (the
                // previous behavior) meant it could steal clicks meant
                // for whichever handle it merely happened to be sitting
                // near, so rockyMassCenterWins settles every other case by
                // actual distance instead.
                if ((LOWORD(wParam) & MK_SHIFT) &&
                    isNear(app->mouseGL, rockyMassCenterWorld, MASS_CENTER_HANDLE_RADIUS) &&
                    rockyMassCenterWins(app->mouseGL, rockyMassCenterWorld, rockyCenter, rockyEdgeHitDown,
                                         app->robotScene.rocky, kneeWorld, footWorld, rockyShin1World, rockyShin2World))
                {
                ...
                // fallback check. But only actually wins when
                // rockyEdgeWins agrees it's the CLOSER of the two (see
                // that function's own comment) -- hitTestRockyEdge's own
        ...
        case WM_MOUSEMOVE:
        {
            ...
            if (app->robotScene.activeKind == ROBOT_KIND_ROCKY)
            {
                ...
                // Body/Leg Weight mass-center dot's CURRENT position -- see
                // rockyMassCenterWins's own comment for why it no longer
                // unconditionally wins the priority chain below.
                PointF rockyMassCenterWorld = computeRockyMassCenterWorld(app->robotScene.rocky);

                // Shin connector-arc handle positions -- moved up ahead of
                // the priority decision below (used to be computed after
                // it) so rockyMassCenterWins has real shin handle points
                // to compare distances against too, not just Body/Edge/
                // Knee/Foot. Same circleAtAxisMid construction as
                // renderer.c's drawRocky and this same WM_MOUSEMOVE's own
                // WM_LBUTTONDOWN hit-test. No stored hoverRockyShin1/2
                // flag, same as Semni's own shin handles -- just a direct
                // isNear check for the label, further below.
                PointF rockyShinAxisMidLocal = { (app->robotScene.rocky.kneeCircle.x + app->robotScene.rocky.footCircle.x) * 0.5f,
                                                  (app->robotScene.rocky.kneeCircle.y + app->robotScene.rocky.footCircle.y) * 0.5f };
                ...
                // segments (Width/Height resize), Knee, Foot -- edges win
                // over Knee/Foot instead of only being checked as a
                // last-resort, mirroring WM_LBUTTONDOWN's own
                // rockyEdgeHitDown ordering. Mass Center only actually
                // wins its own hit-test when Shift is held (see
                // WM_LBUTTONDOWN's own matching Shift check -- dragging
                // the dot is Shift-locked so it doesn't move by accident
                // during ordinary Body-handle drags, given how close the
                // two can sit) AND rockyMassCenterWins agrees it's the
                // CLOSER of the two (see that function's own comment) --
                // computed against whichever of Body/Edge/Knee/Foot/
                // Shin1/Shin2 the mouse is also currently near, rather
                // than unconditionally, so it no longer steals hover from
                // a handle it merely happens to be sitting near. Gating
                // the hover on Shift too (not just the drag) keeps the
                // highlighted handle always matching whatever a click
                // would actually grab.
                int rockyEdgeHitNow = hitTestRockyEdge(app->robotScene.rocky, localMouse);
                BOOL rockyMassHitNow = (LOWORD(wParam) & MK_SHIFT) &&
                    isNear(app->mouseGL, rockyMassCenterWorld, MASS_CENTER_HANDLE_RADIUS);
                BOOL rockyMassWinsNow = rockyMassHitNow &&
                    rockyMassCenterWins(app->mouseGL, rockyMassCenterWorld, rockyCenter, rockyEdgeHitNow,
                                         app->robotScene.rocky, kneeWorld, footWorld, rockyShin1World, rockyShin2World);

                // Same rockyEdgeWins tie-break WM_LBUTTONDOWN now uses --
                // keeps the hover highlight matching whatever a click
                // would actually grab, including a knee/foot/shin handle
                // sitting in a corner an edge's own hit-test slack also
                // reaches into.
                BOOL rockyEdgeWinsNow = (rockyEdgeHitNow != ROCKY_EDGE_NONE) &&
                ...
                {
                    // The body handle IS the whole-body rotation pivot
                    // itself (getRockyCenter just returns bodyX/bodyY
                    // directly) -- unlike every other Rocky handle, which
                    // lives in the rectangle's LOCAL, un-rotated frame and
                    // needs localMouse (the mouse rotated by -angle around
                    // this same pivot) to compare against local bounds,
                    // moving the pivot ITSELF needs the RAW world mouse
                    // position, not a rotated proxy of it -- localMouse
                    // and the real cursor position only coincide when
                    // rocky.angle happens to be exactly 0, so using it
                    // here (as this used to) made the whole body (and the
                    // knee/foot rigidly carried along with it) jump to a
                    // rotated-looking location the instant the robot had
                    // been rotated at all. Knee/foot still use their own
                    // captured offset from the body (set at drag-start),
                    // just added to the raw mouse now instead -- valid
                    // because that offset is a pure rigid translation (the
                    // whole group's angle doesn't change during a body
                    // drag), so no rotation math belongs in any of the
                    // three anymore.
                    app->robotScene.rocky.bodyX = app->mouseGL.x;
                    app->robotScene.rocky.bodyY = app->mouseGL.y;

                    app->robotScene.rocky.kneeCircle.x = app->mouseGL.x + app->rockyDragKneeOffset.x;
                    app->robotScene.rocky.kneeCircle.y = app->mouseGL.y + app->rockyDragKneeOffset.y;

                    app->robotScene.rocky.footCircle.x = app->mouseGL.x + app->rockyDragFootOffset.x;
                    app->robotScene.rocky.footCircle.y = app->mouseGL.y + app->rockyDragFootOffset.y;
                }
        ...
        case WM_MOUSEWHEEL:
        {
            ...
            if (app->robotScene.activeKind == ROBOT_KIND_ROCKY)
            {
                ...
                if (!app->robotScene.rocky.bodyHidden && isNear(mouse, rockyCenter, HIP_HANDLE_RADIUS))
                {
                    ...
                    // The knee (and the foot/shin hanging off it) used
        ...
        case WM_KEYDOWN:
        {
            // Rocky's mass-center handle hover (hoverRockyMassCenter,
            // gating both the yellow highlight and whether a click would
            // actually grab it) is only ever recomputed inside
            // WM_MOUSEMOVE, keyed off (LOWORD(wParam) & MK_SHIFT) -- so if
            // the cursor is already sitting still on top of the handle
            // when Shift goes down, nothing re-evaluates it until the
            // mouse physically moves a pixel, which read as "I have to
            // wiggle the cursor for it to turn yellow". Fix: synthesize a
            // WM_MOUSEMOVE at the CURRENT cursor position (with the
            // CURRENT Shift/mouse-button state folded into wParam, same
            // MK_SHIFT/MK_LBUTTON bits Windows itself would set) so the
            // exact same hover logic re-runs immediately on the down-
            // stroke, with no visible mouse movement required. Harmless
            // for every other robot kind/mode -- it's just a no-op
            // WM_MOUSEMOVE at the same spot the cursor was already at.
            if (wParam == VK_SHIFT)
            {
                POINT shiftCursor;
                GetCursorPos(&shiftCursor);
                ScreenToClient(hwnd, &shiftCursor);

                WPARAM shiftMods = MK_SHIFT;
                if (GetAsyncKeyState(VK_LBUTTON) & 0x8000) shiftMods |= MK_LBUTTON;
                if (GetAsyncKeyState(VK_CONTROL) & 0x8000) shiftMods |= MK_CONTROL;

                SendMessage(hwnd, WM_MOUSEMOVE, shiftMods,
                            MAKELPARAM((short)shiftCursor.x, (short)shiftCursor.y));
            }
        ...
        case WM_KEYUP:
        {
            // Mirror image of WM_KEYDOWN's own VK_SHIFT handling just
            // above -- releasing Shift while the cursor is sitting still
            // over the handle needs the same synthetic WM_MOUSEMOVE to
            // immediately drop the hover/highlight back out (Mass Center
            // hover requires MK_SHIFT to be set at all, so a stale "still
            // yellow after letting go of Shift" would be the mirror-image
            // bug of the one this pair fixes).
            if (wParam == VK_SHIFT)
            {
                POINT shiftCursor;
                GetCursorPos(&shiftCursor);
        ...
        case WM_SIZE:
        {
            ...
            // edit boxes (weightEditW, half the row) rather than
            // stretched across the full content width -- it's a single
            // number, not a ratio pair, so it doesn't need the room and
            // reads oversized left that wide.
            SetWindowPos(app->ui.hActualWeightLabel, NULL,
                 col1X, panelY + relYActualWeight + (btnH - 20) / 2, weightLabelW, 20,
                 SWP_NOZORDER);

            SetWindowPos(app->ui.hActualWeightEdit, NULL,
                 col1X + weightLabelW + colGap, panelY + relYActualWeight + (btnH - 22) / 2, weightEditW, 22,
                 SWP_NOZORDER);
        ...
        case WM_CREATE:
        {
             ...
             // mass-center ratio. This is the number saveRockyAsRobArm
             // actually writes as Rob.txt's own "(gewicht)" weight value.
             app->ui.hActualWeightLabel = CreateWindow(
                L"STATIC",
                L"Weight",
                WS_VISIBLE | WS_CHILD | SS_LEFT,
                0, 0, 10, 10,
             ...
             SendMessage(app->ui.hActualWeightLabel, WM_SETFONT, (WPARAM)g_semniUIFont, TRUE);

             app->ui.hActualWeightEdit = CreateWindowEx(
                WS_EX_CLIENTEDGE,
                L"EDIT",
                L"1.00",   // same two-decimal format refreshWeightEdit always uses
                WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL,
                0, 0, 10, 10,
                hwnd,
                (HMENU)ID_ACTUAL_WEIGHT_EDIT,
                NULL,
                NULL
            );
             SendMessage(app->ui.hActualWeightEdit, WM_SETFONT, (WPARAM)g_semniUIFont, TRUE);
             // see weightEditSubclassProc's own comment -- Enter returns
             // focus to the main window instead of beeping/staying put.
             SetWindowSubclass(app->ui.hActualWeightEdit, weightEditSubclassProc, 0, (DWORD_PTR)hwnd);
            ...
            switch (LOWORD(wParam))
            {
                case ID_ROBOT_SELECTOR:
                    if (HIWORD(wParam) == CBN_SELCHANGE)
                    {
                        ...
                        // Mirror Leg 2 only means anything for Stilo
                        // (its second, independent leg) -- Semni and
                        // Rocky each have exactly one leg, so the button
                        // used to just sit there doing nothing for them
                        // (see ID_MIRROR_LEG2_BUTTON's own comment).
                        // Hide it outright for those two instead, same
                        // "don't show a control that has nothing to act
                        // on" treatment the rest of this panel's kind-
                        // specific controls (Body Wt/Leg Wt/Weight) get.
                ...
                // typed in without needing a redundant GetWindowText of
                // its own first.
                case ID_ACTUAL_WEIGHT_EDIT:
                    // Typing no longer applies live -- the value is confirmed
                    // only when the box loses focus: pressing Enter
                    // (weightEditSubclassProc) or clicking anything else
                    // (canvas, panel, another control). Esc cancels.
