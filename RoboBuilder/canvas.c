...
// High-resolution clock for the [PERF] timings and the settle fast-forward
// budget, in milliseconds (fractional). GetTickCount() only advances every
// ~15.6 ms on Windows, so every duration it measured came out as 0, 15/16
// or 31/32 ms -- too coarse to diagnose a hitch, and it made the 8 ms
// fast-forward budget effectively a 15.6 ms one.
...
// just above the Probe 2 Settle button -- toggled from the View > Show FPS
// menu item (main.c's buildMainMenu, WM_COMMAND's ID_TOGGLE_SHOW_FPS
// handling below). See fpsDisplayValue's own comment for how the number
// itself is measured. Starts TRUE (unlike hideInactiveLayer's own
// "unobtrusive by default" reasoning) -- this exists specifically because
// a real report described the simulation as "sometimes isn't as smooth",
// so it should be visible from the first frame without an extra step,
// rather than requiring the user to already know to turn it on before
// they can even start diagnosing that.
BOOL showFpsOverlay = TRUE;

// Simulation-only console log toggles -- see ui_state.h's own comment and
// the SIM_LOG/SIM_FPS_LOG macros there. Off by default.
BOOL simPhysicsLogEnabled = FALSE;
BOOL simFpsLogEnabled = FALSE;
...
static float simTimeScale = SIMULATION_SLOW_MOTION_SCALE; // Slow Motion is ON by default (by request)
...
// Probe 2 (the knee-bending settle probe in advanceRockySettle, gated by
// rockyProbe2Enabled below) is the source of every settle-pose bug this
// project has chased -- Probe 1's whole-body pivot-around-current-contact
// already implements correct toppling on its own. Rather than deleting
// Probe 2 outright, it's gated behind this checkbox so it can be switched
// off (to compare against pure Probe 1 toppling) without losing the code
...
// Layout constants for hProbe2ToggleBtn (WM_CREATE/WM_SIZE below) -- kept
...
#define PROBE2_TOGGLE_WIDTH_PX  150
#define PROBE2_TOGGLE_HEIGHT_PX 28
#define PROBE2_TOGGLE_MARGIN_PX 10

// Vertical space reserved above hProbe2ToggleBtn, at the same top-right
// corner, for the FPS counter (View > Show FPS, ID_TOGGLE_SHOW_FPS) drawn
// by canvasRenderFrame -- see fpsDisplayValue's own comment. Reserved
// UNCONDITIONALLY (the button is offset down by this amount regardless of
// whether the overlay is currently switched on) rather than only when
// showFpsOverlay is TRUE, so toggling it on/off never shifts the button
// and everything else pinned below it -- it would be jarring for Probe 2
// Settle to visibly jump position every time this is toggled.
#define FPS_OVERLAY_RESERVED_HEIGHT_PX 24

// How often (real milliseconds) the FPS counter's displayed number is
// recomputed -- see fpsDisplayValue's own comment for why this isn't just
// updated every single frame.
#define FPS_UPDATE_INTERVAL_MS 500

static HWND hProbe2ToggleBtn = NULL;

// Backing state for hProbe2ToggleBtn -- see its comment just above and
// the entry-condition check in advanceRockySettle. TRUE = Probe 2 runs
// normally (pre-existing behavior); FALSE = advanceRockySettle skips
// straight to Probe 1's body-pivot toppling, never bending the knee.
static BOOL rockyProbe2Enabled = TRUE;
        ...
        for (int i = 0; i < count - 1; i++)
        {
            ...
            if (dist < combinedRadius)
            {
                if (!gSuppressGravityDebug)
                    SIM_LOG("[COLLIDE] seg=%d line=(%.5f,%.5f)-(%.5f,%.5f) pt=(%.5f,%.5f) dist=%.5f eRadius=%.5f combined=%.5f\n",
                                                  ...
                                                  float eRadius, float* outT)
{
    ...
    for (int s = 0; s < canvas.segmentResultCount; s++)
    {
        ...
        for (int i = 0; i < count - 1; i++)
        {
            ...
            {
                if (!gSuppressGravityDebug)
                    SIM_LOG("[COLLIDE] seg=%d line=(%.5f,%.5f)-(%.5f,%.5f) edge=(%.5f,%.5f)-(%.5f,%.5f) dist=%.5f eRadius=%.5f combined=%.5f\n",
...
// distinction matters: Probe 2's knee bend rotates the WHOLE shin (arc
...
// already uses (see e.g. Probe 2's own baseFootClearance just below).
...
// pivot Probe 1's rotation around; rockyDanglingPointClearance wants the
// second-closest, the "next in line" contact Probe 1's rotation is actually
...
// noise. Since Probe 1 pivots around whatever rockyTopplePivotWorld
...
// for real collision.
// Defined further down with the robot-generic settle accessors.
static void semniContactCandidates(const Semni* sm, BOOL legPart, RockyLegContact* best, RockyLegContact* second);
    ...
    switch (app.robotScene.activeKind)
    {
        case ROBOT_KIND_ROCKY:
        {
            if (!app.robotScene.rocky.legHidden)
            {
                ...
                // point rockyTopplePivotWorld/Probe 1 rotate around) --
        ...
        default:
        {
            // Same overlay as Rocky's case above (it used to draw one tiny dot
            // at each circle's CENTER, which is why no contact ever showed on
            // Semni's circles): exact rim contact points via
            // findCircleContactDirections, the current topple pivot in red
            // (drawn last, on top), every other touching point in orange,
            // cyan "sampled but not touching" dots around each circle and
            // along each fillet.
            //
            // Pivot = the lowest of Semni's leg/body contact candidates (the
            // same candidates Probe 1 pivots around -- semniContactCandidates).
            // Deliberately NOT settleToppleContactPoints: that updates the
            // settle's leg/body hysteresis state, and drawing must never
            // change what the simulation does.
            RockyLegContact legBest, legSecond, bodyBest, bodySecond;
            semniContactCandidates(&app.robotScene.robot, TRUE, &legBest, &legSecond);
            semniContactCandidates(&app.robotScene.robot, FALSE, &bodyBest, &bodySecond);
            RockyLegContact pivotContact = (legBest.clearance <= bodyBest.clearance) ? legBest : bodyBest;
            ...
            for (int c = 0; c < NUM_ROBOT_BODY_CIRCLES; c++)
            {
                ...
                // Circle candidates carry the circle's own center as .world
                // (same computeSemniBodyCircles call), so exact equality is safe.
                BOOL isPivotCircle = (pivotContact.kind != ROCKY_LEG_CONTACT_ARC
                                      && pivotContact.world.x == bodyCircles[c].center.x
                                      && pivotContact.world.y == bodyCircles[c].center.y);
                ...
                for (int k = 0; k < contactCount; k++)
                {
                    ...
                    if (isPivotCircle)
                    {
                        if (!pivotHasContact) { pivotHasContact = TRUE; pivotDrawX = rx; pivotDrawY = ry; }
                    }
                    else
                    {
                        drawMarkerDisc(rx, ry, liveContactDotRadiusNonPivot, 1.0f, 0.55f, 0.0f, 1.0f);
                ...
                PointF circleSamplePts[ARC_SAMPLE_COUNT];
                computeCircleSamplePoints(bodyCircles[c].center, bodyCircles[c].radius, circleSamplePts);
                for (int sIdx = 0; sIdx < ARC_SAMPLE_COUNT; sIdx++)
                {
                    float secx, secy;
                    robotPointToEnvWorld(circleSamplePts[sIdx].x, circleSamplePts[sIdx].y, &secx, &secy);
                    if (!pointVisuallyContactsEnvironmentStroke(secx, secy, eArcThickness))
                        drawMarkerDisc(circleSamplePts[sIdx].x, circleSamplePts[sIdx].y, liveMissDotRadius, liveMissR, liveMissG, liveMissB, liveMissA);
                }
            }
            ...
            for (int a = 0; a < NUM_ROBOT_CIRCLE_SEGMENTS; a++)
            {
                for (int i = 0; i < arcCounts[a]; i++)
                {
                    ...
                    if (pointVisuallyContactsEnvironmentStroke(ecx, ecy, eArcThickness))
                    {
                        // Arc candidates carry the exact sample as .world
                        // (same computeSemniArcPoints call) -- the seam arcs'
                        // kind is BODY_CORNER, so match by position only.
                        BOOL isPivotSample = (pivotContact.world.x == arcPts[a][i].x
                                              && pivotContact.world.y == arcPts[a][i].y);
                        if (isPivotSample)
                        {
                            pivotHasContact = TRUE;
                            pivotDrawX = arcPts[a][i].x;
...
// Semni only: Probe 2's step size for the HIP joint (rockyKneeSettleStep
// above doubles as Semni's KNEE step). The rocky* settle state variables are
// shared by whichever robot is settling -- Rocky and Semni both run
// advanceRockySettle now; only one robot is ever active at a time.
static float semniHipSettleStep = 0.0f;
...
// moving: Probe 2 bends the knee based on the body angle Probe 1 set last
// tick, then Probe 1 pivots the body based on the knee angle Probe 2 just
...
// Probe 2 went quiet (legRestingOnImmovableJoint) and Probe 1 alone kept
...
// commit yet, or just reset by a fresh landing) that Probe 1 actually
...
// before any of Probe 1's three direct branches (or the scout
...
// becoming a NEW deadlock of its own: once Probe 1 had already pivoted
...
// WM_KEYDOWN kickoff below) to stop Probe 2 (knee-bending) from firing
// while the USER is the one actively driving kneeAngle -- without this,
// Probe 2's own "improve the resting fit" search runs every 16ms via the
...
// then Probe 2 bends it back and drops the body again a tick later),
// which is what read as the body visibly jittering up and down while
// holding E or Q. Probe 1 (body-rotate/topple) and the plain gravity
...
// where nothing else is driving kneeAngle and Probe 2 is safe to run.
...
// dangling foot should keep Probe 2 actively trying to bring it down
...
// Shared by Rocky (rockyToppleContactPoints) and Semni (settleToppleContactPoints):
// picks the pivot/second contact out of the LEG category's best two and the
// BODY category's best two, with the same leg-vs-body hysteresis
// (rockyToppleLastPivotCategory) so the pivot doesn't flip categories on
// tiny numeric noise.
static void combineToppleCategories(BOOL haveLeg, RockyLegContact legBest, RockyLegContact legSecond,
                                    BOOL haveBody, RockyLegContact bodyBest, RockyLegContact bodySecond,
                                    RockyLegContact* closest, RockyLegContact* secondClosest)
{
    BOOL useLeg;
    if (!haveLeg)
        useLeg = FALSE;
...
static void rockyToppleContactPoints(const Rocky* r, RockyLegContact* closest, RockyLegContact* secondClosest)
{
    ...
    // switching.
    combineToppleCategories(haveLeg, legBest, legSecond, haveBody, bodyBest, bodySecond, closest, secondClosest);
}
...
// above, every time Probe 1 below tries a candidate rotation. `pivot` is
...
// a real report showed: Probe 1 never committing a single rotation,
...
// ---- Robot-generic settle accessors (Rocky + Semni) ----
//
// advanceRockySettle's Probe 2 (passive joint swing) and Probe 1 (whole-body
// topple around a ground contact) were written against Rocky's struct
// directly. These give it the same handful of things for whichever robot is
// active, so Semni runs the exact same settle logic:
//   body angle + rotation center, center of mass, hidden flags,
//   Probe 2 joints (Rocky: knee; Semni: knee then hip) and the foot's
//   clearance, and the contact candidates Probe 1 pivots around.
// Semni's contact categories mirror Rocky's: LEG = hip/knee/foot circles +
// thigh/shin arcs (hip circle plays the role of Rocky's body-fixed knee
// circle), BODY = head/butt circles + seam arcs (Rocky's rectangle corners).

static BOOL settleSupportedKind(void)
{
    return app.robotScene.activeKind == ROBOT_KIND_ROCKY || app.robotScene.activeKind == ROBOT_KIND_SEMNI;
}

static float* settleBodyAnglePtr(void)
{
    return (app.robotScene.activeKind == ROBOT_KIND_ROCKY) ? &app.robotScene.rocky.angle : &app.robotScene.robot.angle;
}

static PointF settleBodyCenter(void)
{
    return (app.robotScene.activeKind == ROBOT_KIND_ROCKY) ? getRockyCenter(app.robotScene.rocky) : getCenter(app.robotScene.robot);
}

static PointF settleMassCenter(void)
{
    return computeSimulationMassCenterWorld(&app);
}

static BOOL settleLegHidden(void)
{
    return (app.robotScene.activeKind == ROBOT_KIND_ROCKY) ? app.robotScene.rocky.legHidden : FALSE;
}

static BOOL settleBodyHidden(void)
{
    return (app.robotScene.activeKind == ROBOT_KIND_ROCKY) ? app.robotScene.rocky.bodyHidden : FALSE;
}

static int settleJointCount(void)
{
    return (app.robotScene.activeKind == ROBOT_KIND_ROCKY) ? 1 : 2;
}

static float* settleJointPtr(int j)
{
    if (app.robotScene.activeKind == ROBOT_KIND_ROCKY) return &app.robotScene.rocky.kneeAngle;
    return (j == 0) ? &app.robotScene.robot.kneeAngle : &app.robotScene.robot.hipAngle;
}

static float* settleJointStepPtr(int j)
{
    return (j == 0) ? &rockyKneeSettleStep : &semniHipSettleStep;
}

static const char* settleJointName(int j)
{
    if (app.robotScene.activeKind == ROBOT_KIND_ROCKY) return "kneeAngle";
    return (j == 0) ? "kneeAngle" : "hipAngle";
}

static BOOL settleAllJointStepsBelow(float minStep)
{
    for (int j = 0; j < settleJointCount(); j++)
        if (*settleJointStepPtr(j) >= minStep) return FALSE;
    return TRUE;
}

// Clearance (env world units) of the foot circle -- the end of the leg that
// Probe 2's "swing the dangling foot closer" criterion measures.
static float settleFootClearance(void)
{
    CircleSegment foot;
    if (app.robotScene.activeKind == ROBOT_KIND_ROCKY)
    {
        CircleSegment c[NUM_ROCKY_BODY_CIRCLES];
        computeRockyBodyCircles(app.robotScene.rocky, c);
        foot = c[1];
    }
    else
    {
        CircleSegment c[NUM_ROBOT_BODY_CIRCLES];
        computeSemniBodyCircles(app.robotScene.robot, c);
        foot = c[4];
    }
    float ecx, ecy;
    robotPointToEnvWorld(foot.center.x, foot.center.y, &ecx, &ecy);
    return nearestEnvDistance(ecx, ecy) - (robotLengthToEnvWorld(foot.radius) + simEnvLineHalfWidthWorld());
}

static void settleInsertCandidate(RockyLegContact cand, RockyLegContact* best, RockyLegContact* second)
{
    if (cand.clearance < best->clearance) { *second = *best; *best = cand; }
    else if (cand.clearance < second->clearance) { *second = cand; }
}

// Best/second contact candidates on Semni, from either its LEG parts
// (legPart TRUE) or its BODY parts -- see the category note above.
static void semniContactCandidates(const Semni* sm, BOOL legPart, RockyLegContact* best, RockyLegContact* second)
{
    RockyLegContact none = { { 0.0f, 0.0f }, 1e9f, legPart ? ROCKY_LEG_CONTACT_KNEE : ROCKY_LEG_CONTACT_BODY_CORNER };
    *best = none;
    *second = none;

    float halfW = simEnvLineHalfWidthWorld();

    CircleSegment circles[NUM_ROBOT_BODY_CIRCLES];
    computeSemniBodyCircles(*sm, circles);
    for (int c = 0; c < NUM_ROBOT_BODY_CIRCLES; c++)
    {
        BOOL isLeg = (c >= 2); // 0 head, 1 butt | 2 hip, 3 knee, 4 foot
        if (isLeg != legPart) continue;
        float ecx, ecy;
        robotPointToEnvWorld(circles[c].center.x, circles[c].center.y, &ecx, &ecy);
        RockyLegContact cand;
        cand.world = circles[c].center;
        cand.clearance = nearestEnvDistance(ecx, ecy) - (robotLengthToEnvWorld(circles[c].radius) + halfW);
        cand.kind = !legPart ? ROCKY_LEG_CONTACT_BODY_CORNER : (c == 4 ? ROCKY_LEG_CONTACT_FOOT : ROCKY_LEG_CONTACT_KNEE);
        settleInsertCandidate(cand, best, second);
    }

    float arcPad = robotLengthToEnvWorld(SIMULATION_ARC_COLLISION_THICKNESS) + halfW;
    PointF arcPts[NUM_ROBOT_CIRCLE_SEGMENTS][ARC_SAMPLE_COUNT];
    int arcCounts[NUM_ROBOT_CIRCLE_SEGMENTS];
    computeSemniArcPoints(*sm, arcPts, arcCounts);
    for (int a = 0; a < NUM_ROBOT_CIRCLE_SEGMENTS; a++)
    {
        BOOL isLeg = (a >= 2); // 0-1 seam | 2-3 thigh, 4-5 shin
        if (isLeg != legPart) continue;
        for (int i = 0; i < arcCounts[a]; i++)
        {
            float ecx, ecy;
            robotPointToEnvWorld(arcPts[a][i].x, arcPts[a][i].y, &ecx, &ecy);
            RockyLegContact cand;
            cand.world = arcPts[a][i];
            cand.clearance = nearestEnvDistance(ecx, ecy) - arcPad;
            cand.kind = legPart ? ROCKY_LEG_CONTACT_ARC : ROCKY_LEG_CONTACT_BODY_CORNER;
            settleInsertCandidate(cand, best, second);
        }
    }
}

// Closest/second-closest LEG contact (Rocky: rockyLegContactPoints).
static void settleLegContactPoints(RockyLegContact* closest, RockyLegContact* secondClosest)
{
    if (app.robotScene.activeKind == ROBOT_KIND_ROCKY)
        rockyLegContactPoints(&app.robotScene.rocky, closest, secondClosest);
    else
        semniContactCandidates(&app.robotScene.robot, TRUE, closest, secondClosest);
}

// Pivot + dangling point over the WHOLE robot (Rocky: rockyToppleContactPoints).
static void settleToppleContactPoints(RockyLegContact* closest, RockyLegContact* secondClosest)
{
    if (app.robotScene.activeKind == ROBOT_KIND_ROCKY)
    {
        rockyToppleContactPoints(&app.robotScene.rocky, closest, secondClosest);
        return;
    }
    RockyLegContact legBest, legSecond, bodyBest, bodySecond;
    semniContactCandidates(&app.robotScene.robot, TRUE, &legBest, &legSecond);
    semniContactCandidates(&app.robotScene.robot, FALSE, &bodyBest, &bodySecond);
    combineToppleCategories(TRUE, legBest, legSecond, TRUE, bodyBest, bodySecond, closest, secondClosest);
}

static PointF settleTopplePivotWorld(void)
{
    if (settleLegHidden() && settleBodyHidden())
        return settleBodyCenter();
    RockyLegContact closest, secondClosest;
    settleToppleContactPoints(&closest, &secondClosest);
    return closest.world;
}

static float settleDanglingPointClearance(void)
{
    RockyLegContact closest, secondClosest;
    settleToppleContactPoints(&closest, &secondClosest);
...
// Rotates the whole body by deltaAngle around `pivot` (a world point,
// captured before the call) instead of around the body's own center: a
// plain angle change only ever rotates around the center, so this follows
// it with a compensating translate that puts `pivot` back where it was --
// the standard "rotate about center, then translate" trick. Returns the
// (dx, dy) shift applied, so a caller can undo it exactly (translate by
// its negation, then restore the angle) without re-deriving anything.
// Works for whichever robot is active (Rocky or Semni).
static PointF rotateActiveBodyAroundPivot(PointF pivot, float deltaAngle)
{
    PointF center = settleBodyCenter();
    PointF pivotAfterSpin = rotatePoint(pivot, center, deltaAngle);
    PointF shift = { pivot.x - pivotAfterSpin.x, pivot.y - pivotAfterSpin.y };

    *settleBodyAnglePtr() += deltaAngle;
    translateActiveRobot(shift.x, shift.y);
    return shift;
}

// Halves rockyBodySettleStep for a reversal-guard block, but never below
// SIMULATION_LEG_SETTLE_MIN_STEP_DEG. The guard is meant to give up after
// SIMULATION_ROCKY_BODY_REVERSAL_BLOCK_STREAK consecutive blocks and let
// the reversal through -- but a plain halving on every block (plus the
// pivot-side-flip halving) took a 1 deg step below the floor in 7 halvings,
// so Probe 1 stopped running before the guard ever released, and the
// settle "converged" with the body still visibly toppling (real report:
// Rocky balanced on its foot with the center of mass far to one side).
static void shrinkRockyBodyStepForReversalGuard(void)
{
...
// ---- Final "seat the dangling point" pass (run once, right before
// advanceRockySettle latches rockySettleConverged) ----
//
// Probe 1 stops once its step size falls below
// SIMULATION_LEG_SETTLE_MIN_STEP_DEG, and that alone doesn't guarantee
// the second contact point (e.g. the foot, while the knee is the pivot)
// actually reached the ground. A real report showed a Rocky resting on
// its knee with the foot visibly flush but NOT registering as a contact
// (no orange dot): it was hovering a sub-pixel gap above the line, just
// past SIMULATION_CONTACT_VISUAL_TOLERANCE. Same fix applyGravityStep
// already uses for a vertical drop: binary-search the remaining rotation
// around the pivot for the largest angle that does NOT collide, so the
// dangling point converges onto the surface instead of stopping
// wherever the last whole step happened to land.
//
// Only fires for a SMALL remaining gap (SIMULATION_ROCKY_SEAT_MAX_GAP) --
// a point genuinely dangling far above the ground is left alone -- and
// only commits if the rotation doesn't raise the center of mass (a
// physically real rest pose never needs to climb to close a gap).
//
// Unlike Probe 1, a candidate here is NOT allowed to "fix" a collision by
// lifting the whole robot (clearPivotRoundingResidual's full
// SIMULATION_SLOPE_CORRECTION_MAX budget): any lift bigger than
// SIMULATION_ROCKY_SEAT_MAX_PIVOT_LIFT counts as blocked. Otherwise a
// dangling point pushed INTO the ground would just be "cured" by lifting
// the pivot off it instead -- trading one hovering contact for another.

// Rotates by deltaDeg around pivot, measures, then fully undoes itself.
// Returns TRUE if that orientation is blocked (collides beyond a
// rounding-sized pivot lift, or the dangling point went below zero).
static BOOL rockySeatProbe(PointF pivot, float deltaDeg,
                           float* outDanglingClearance, float* outComY)
{
    float* bodyAngle = settleBodyAnglePtr();
    float baseAngle = *bodyAngle;
    PointF shift = rotateActiveBodyAroundPivot(pivot, deltaDeg);

    // Dangling point below the surface -> blocked, no collision test needed.
    float clearance = settleDanglingPointClearance();
    BOOL blocked = (clearance < 0.0f);

    // Otherwise: a collision here is either float rounding at the pivot
    // (fine) or something real. Instead of clearPivotRoundingResidual's
    // 12-step search for the exact lift (this made the whole seat pass
    // cost ~60-70 ms -- real [PERF] report), just test the one lift we'd
    // accept: if lifting by SIMULATION_ROCKY_SEAT_MAX_PIVOT_LIFT clears
    // it, it was rounding; if not, it's blocked. At most 2 checks.
    float lift = 0.0f;
    if (!blocked && robotCollidesWithEnvironment())
    {
        lift = SIMULATION_ROCKY_SEAT_MAX_PIVOT_LIFT;
        translateActiveRobot(0.0f, lift);
        if (robotCollidesWithEnvironment())
            blocked = TRUE;
    }

    if (outDanglingClearance) *outDanglingClearance = clearance;
    if (outComY) *outComY = settleMassCenter().y;
...
static void seatRockyDanglingPoint(void)
{
    if (settleLegHidden() && settleBodyHidden())
        return;

    BOOL savedSuppress = gSuppressGravityDebug;
    gSuppressGravityDebug = TRUE; // this search calls robotCollidesWithEnvironment dozens of times

    RockyLegContact pivotContact, dangling;
    settleToppleContactPoints(&pivotContact, &dangling);

    float envPerRobot = robotLengthToEnvWorld(1.0f);
    float gapRobot = (envPerRobot > 0.0f) ? dangling.clearance / envPerRobot : 0.0f;

    // Skip when the gap is already small enough to register as a visible
    // contact (half of SIMULATION_CONTACT_VISUAL_TOLERANCE, for margin) --
    // nothing a viewer could see would change, and most convergences land
    // here, so this also avoids the whole search's cost.
    if (dangling.clearance <= 0.0f
        || gapRobot <= 0.5f * SIMULATION_CONTACT_VISUAL_TOLERANCE
        || gapRobot > SIMULATION_ROCKY_SEAT_MAX_GAP)
    {
        gSuppressGravityDebug = savedSuppress;
        return; // already touching (or visibly so), or genuinely dangling -- nothing to seat
    }

    PointF pivot = pivotContact.world;
    float lx = dangling.world.x - pivot.x;
    float ly = dangling.world.y - pivot.y;
    float lever = sqrtf(lx * lx + ly * ly);
    if (lever < 1e-5f)
    {
        gSuppressGravityDebug = savedSuppress;
        return;
    ...
    // Rotation (deg) that would close the gap if the dangling point moved
    // straight down; doubled so the bracket's far end is safely past it.
    float spanDeg = 2.0f * (gapRobot / lever) * (180.0f / 3.14159265f);
    if (spanDeg > SIMULATION_BODY_SETTLE_STEP_DEG) spanDeg = SIMULATION_BODY_SETTLE_STEP_DEG;
    if (spanDeg < 1e-4f) spanDeg = 1e-4f;

    float baseComY = settleMassCenter().y;

    // Which way lowers the dangling point? Try a small nudge each way.
    float cMinus = 0.0f, cPlus = 0.0f;
    BOOL bMinus = rockySeatProbe(pivot, -0.1f * spanDeg, &cMinus, NULL);
    BOOL bPlus  = rockySeatProbe(pivot,  0.1f * spanDeg, &cPlus, NULL);
    int dir = 0;
    if (!bMinus && cMinus < dangling.clearance && (bPlus || cMinus <= cPlus)) dir = -1;
    else if (!bPlus && cPlus < dangling.clearance) dir = 1;

    if (dir == 0)
    {
        SIM_LOG("[SETTLE] seat: dangling kind=%d gap=%.6f (robot units) -- neither rotation direction lowers it, leaving as is\n",
               (int)dangling.kind, gapRobot);
        gSuppressGravityDebug = savedSuppress;
        return;
    }

    if (!rockySeatProbe(pivot, dir * spanDeg, NULL, NULL))
    {
        SIM_LOG("[SETTLE] seat: dangling kind=%d gap=%.6f -- still not blocked %.5f deg out, leaving as is (not a small residual gap)\n",
               (int)dangling.kind, gapRobot, dir * spanDeg);
        gSuppressGravityDebug = savedSuppress;
        return;
    }

    float safe = 0.0f, blockedAt = spanDeg;
    for (int i = 0; i < SIMULATION_ROCKY_SEAT_SEARCH_ITERATIONS; i++)
    {
        float mid = 0.5f * (safe + blockedAt);
        if (rockySeatProbe(pivot, dir * mid, NULL, NULL)) blockedAt = mid;
        else                                                  safe = mid;
    }

    float seatedClearance = 0.0f, seatedComY = baseComY;
    rockySeatProbe(pivot, dir * safe, &seatedClearance, &seatedComY);

    if (seatedComY > baseComY + 1e-6f)
    {
        SIM_LOG("[SETTLE] seat: dangling kind=%d gap=%.6f -- closing it would RAISE the center of mass by %.6f, leaving as is\n",
               (int)dangling.kind, gapRobot, seatedComY - baseComY);
        gSuppressGravityDebug = savedSuppress;
        return;
    }

    rotateActiveBodyAroundPivot(pivot, dir * safe);
    float lift = 0.0f;
    if (robotCollidesWithEnvironment())
        lift = clearPivotRoundingResidual();

    float seatedGapRobot = (envPerRobot > 0.0f) ? seatedClearance / envPerRobot : 0.0f;
    SIM_LOG("[SETTLE] seat: pivot kind=%d, dangling kind=%d gap %.6f -> %.6f (robot units) after rotating %.6f deg (pivot lift %.6f)%s\n",
           (int)pivotContact.kind, (int)dangling.kind, gapRobot, seatedGapRobot, dir * safe, lift,
           (seatedGapRobot > SIMULATION_CONTACT_VISUAL_TOLERANCE)
               ? " -- STILL above tolerance: something else (arc/body edge) blocked the rotation first"
               : "");

    gSuppressGravityDebug = savedSuppress;
}

// [PERF] breakdown of the most recent advanceRockySettle call's two
// convergence-only extras (the sanity-check scout and the seat pass) --
// reset at the top of every call, read by the settle fast-forward's
// [PERF] line so a slow call can be attributed. 0 when that part didn't run.
static double perfLastSettleSanityMs = 0.0;
static double perfLastSettleSeatMs = 0.0;

// Naming (by request): Probe 1 = TOPPLING (whole-body rotation around a
// ground contact), Probe 2 = PASSIVE JOINT SWING (knee / hip). Execution
// order inside each settle call is unchanged from before the rename:
// Probe 2 (joint swing) runs first, then Probe 1 (toppling) -- the numbers
// are names only, not the order they run in.
static void advanceRockySettle(void)
{
    perfLastSettleSanityMs = 0.0;
    perfLastSettleSeatMs = 0.0;

    if (!settleSupportedKind())
        return; // Rocky and Semni only -- see the robot-generic settle accessors
    ...
    if (semniHipSettleStep <= 0.0f)
        semniHipSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;

    // Whichever robot is settling (Rocky or Semni) -- see the robot-generic
    // settle accessors. These stand in for the old Rocky* r fields.
    float* bodyAngle = settleBodyAnglePtr();
    BOOL legHidden = settleLegHidden();
    BOOL bodyHidden = settleBodyHidden();

    // Whether the leg's CURRENT ground contact is somewhere Probe 2 should
    // leave alone -- if so, Probe 2 below is skipped for this tick. Covers
    ...
    // the gap Probe 2 itself just pried open -- not genuine progress, just
    // Probe 2 repeatedly prying a real, already-settled arc contact loose
    ...
    // Probe 2 (its clearance checks "judge that fairly", per an earlier
    ...
    // degree each, tick after tick, seemingly forever -- Probe 2 kept
    // finding tiny "progress" swinging the dangling foot around the
    // now-fixed knee, and Probe 1 kept finding tiny "progress" pivoting
    ...
    // why this used to be the one contact Probe 2 was still allowed to
    ...
    // the foot itself is genuinely resting (clearance already ~0), Probe 2
    // leaves it alone too, freezing kneeAngle at whatever angle the foot
    // happened to land at. This does NOT block Probe 2's other,
    ...
    //
    // Probe 1's body rotation (pivoting the whole assembly around whatever
    ...
    // -- so Probe 2 just steps aside until none of them is true anymore.
    ...
    // Probe 2 whenever a body corner was the immovable pivot): a real
    // report showed kneeAngle winding past -280 degrees with a corner
    // long since settled as Probe 1's pivot, which looked at first like
    ...
    // the frozen point and the point Probe 2 moves are THE SAME point --
    ...
    // fixed; that's the whole "does NOT block Probe 2's still-legitimate
    ...
    // -- silencing Probe 2 before it ever got to swing the foot down at
    ...
    // diagnostic printf below (Probe 2's own else-branch, further down)
    ...
    if (!legHidden)
    {
        ...
        settleLegContactPoints(&currentContact, &currentSecondContact);
    ...
    // TRUE for this tick only when one of Probe 1's four reversal-guard
    // branches actually fired (bestDrop/bestComDrop/bestClearanceGain's
    // direct checks, or the scout fallback's own copy) -- every one of
    // those branches only fires when a real, above-threshold improvement
    // WAS found in the blocked direction, just set aside for one tick to
    // stop it bouncing against last tick's own commit. That is the
    // opposite of "no further improvement exists": real improvement was
    // found and deliberately deferred. Used below to stop the convergence
    // check from latching TRUE on the exact tick this fires -- a real
    // report showed the guard's own blocked-streak reaching its release
    // threshold (SIMULATION_ROCKY_BODY_REVERSAL_BLOCK_STREAK) on the SAME
    // tick rockyBodySettleStep's repeated halving (one halving per blocked
    // tick, six of them by the time the streak reaches its threshold)
    // pushed it below SIMULATION_LEG_SETTLE_MIN_STEP_DEG -- convergence
    // fired that same tick and froze the loop for good, so the guard's own
    // release (which was only ever going to take effect the FOLLOWING
    // tick, once its own "< streak" check finally reads false) never
    // actually got to run. The convergence sanity check just above this
    // comment's own block didn't catch that case either: it only scouts a
    // single full-size SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG probe, and the
    // genuine improvement the guard was blocking here was only real at the
    // tiny, already-six-times-halved step size Probe 1 was actually
    // using -- a full-size probe overshoots straight past it into a worse
    // pose and reports no improvement at all. This flag catches that case
    // directly instead, by watching whether a real improvement was found
    // and blocked on this exact tick, regardless of step size.
    BOOL rockyBodyReversalGuardBlockedThisTick = FALSE;
    ...
    // Probe 2: bend the knee. Skipped entirely while the leg is hidden
    ...
    // just above), and while rockyProbe2Enabled is FALSE -- the top-right
    // "Probe 2 Settle" checkbox (WM_CREATE/WM_COMMAND's ID_PROBE2_TOGGLE)
    // lets this whole probe be switched off at runtime, leaving only
    // Probe 1's whole-body pivot-around-current-contact toppling active,
    // without deleting any of this code.
    // Probe 2 runs once per settle joint: Rocky's knee, or Semni's knee
    // then hip (each with its own step size -- settleJointStepPtr). Both
    // use the same foot-clearance criterion (settleFootClearance).
    for (int settleJoint = 0; settleJoint < settleJointCount(); settleJoint++)
    {
    // At most ONE joint commits per settle call -- otherwise Semni (knee +
    // hip) could drop the body twice per call where Rocky (knee only) drops
    // once, settling twice as fast. The other joint gets its turn next call.
    if (probe2Committed)
        break;
    float* joint = settleJointPtr(settleJoint);
    float* jointStep = settleJointStepPtr(settleJoint);
    if (!legHidden && !rockyKneeSettleSuppressed && !legRestingOnImmovableJoint && rockyProbe2Enabled && (*jointStep) >= SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
    {
        float baseKneeAngle = (*joint);
        float bestDrop = 0.0f;
        ...
        float baseFootClearance = settleFootClearance();

        float bestClearanceGain = 0.0f;
        float bestClearanceKneeAngle = baseKneeAngle;

        // Per-direction values, same reason Probe 1 keeps its own
        ...
        {
            (*joint) = baseKneeAngle + dir * (*jointStep);
            ...
            if (!robotCollidesWithEnvironment())
            {
                float clearance = settleFootClearance();
        ...
        // Probe 1's own directionsStraddle just below for the mirror
        ...
        {
            SIM_LOG("[SETTLE] %s=%.2f both directions improve but by different criteria (drop=%.5f/%.5f gain=%.5f/%.5f) -- shrinking instead of picking one, to avoid rocking back and forth\n",
                   settleJointName(settleJoint), (*joint), dropAtDir[0], dropAtDir[1], gainAtDir[0], gainAtDir[1]);
            (*jointStep) *= 0.5f;
        }
        else if (bestDrop > SIMULATION_LEG_SETTLE_MIN_DROP)
        {
            ...
            SIM_LOG("[SETTLE] %s=%.2f bent the knee %.4f deg, body fell %.5f further this tick (of %.5f available)\n",
                   settleJointName(settleJoint), (*joint), (*jointStep), appliedDrop, bestDrop);
        ...
        else if (bestClearanceGain > SIMULATION_LEG_SETTLE_MIN_CLEARANCE_GAIN)
        {
            ...
            (*joint) = bestClearanceKneeAngle;
            probe2Committed = TRUE;
            probe2DirectCommit = TRUE;
            SIM_LOG("[SETTLE] %s=%.2f swung the dangling foot %.4f deg closer to the ground (clearance improved %.5f, body stayed put)\n",
                   settleJointName(settleJoint), (*joint), (*jointStep), bestClearanceGain);
        }
        else
        {
            ...
            // just one ordinary (*jointStep)-sized nudge, so this
            ...
            {
                (*joint) = baseKneeAngle + dir * SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG;
                ...
                if (scoutClear)
                {
                    float sClearance = settleFootClearance();
                ...
                (*joint) = baseKneeAngle; // undo this trial, same as the direct branch's own undo above
            }
            ...
            if (scoutWinner != -1)
            {
                ...
                SIM_LOG("[SETTLE] %s=%.2f scouted real improvement %.1f deg out (drop=%.5f clearanceGain=%.5f), fell %.5f this tick -- nudging that way instead of shrinking\n",
                       settleJointName(settleJoint), (*joint), dir * SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG, scoutDrop, scoutGain, nudgeAppliedDrop);
                scoutedKnee = TRUE;
                probe2Committed = TRUE;
            }
            ...
            {
                (*joint) = baseKneeAngle;
                (*jointStep) *= 0.5f; // nothing here, and nothing further out either -- get finer before giving up
            }
        }
    }
    else if (!legHidden)
    {
        // Diagnostic only, temporary -- a real report showed Probe 2
        // staying completely silent (no [SETTLE] kneeAngle=... lines at
        // all) for long stretches while Probe 1 alone kept pivoting the
        ...
        SIM_LOG("[SETTLE] Probe 2 (%s) skipped this tick -- suppressed=%d legRestingOnImmovableJoint=%d(kind=%d clearance=%.5f) probe2Enabled=%d step=%.5f (min=%.5f)\n",
               settleJointName(settleJoint), rockyKneeSettleSuppressed, legRestingOnImmovableJoint,
               legRestingOnImmovableJoint ? (int)pivotKindForDiagnostic : -1,
               legRestingOnImmovableJointClearanceForDiagnostic,
               rockyProbe2Enabled, (*jointStep), SIMULATION_LEG_SETTLE_MIN_STEP_DEG);
    }
    } // end per-joint Probe 2 loop

    // Probe 1: rotate the rectangle itself. This used to just change
    // (*bodyAngle) directly, which always rotates around (bodyX, bodyY) --
    ...
    if (rockyBodySettleStep >= SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
    {
        float baseBodyAngle = (*bodyAngle);
        PointF pivot = settleTopplePivotWorld(); // same contact point for both directions below

        // The anti-oscillation machinery below (pivot-side-flip halving and
        // the reversal guard) is only meant for a body rocking back and
        // forth near BALANCE, where the center of mass sits almost over the
        // pivot. When the center of mass is far off to one side of the only
        // contact, the torque direction is unambiguous -- the body simply
        // has to topple that way -- so blocking or shrinking that move just
        // freezes an impossible pose (see SIMULATION_ROCKY_REVERSAL_GUARD_MAX_COM_OFFSET).
        //
        // "Far from the pivot" alone isn't enough, though: resting on TWO
        // contacts (e.g. knee AND foot both on the ground) with the center
        // of mass BETWEEN them is a stable rest pose even though the center
        // of mass is far from either one. There, no rotation is physically
        // possible, and without the guard Probe 1 just alternates pivots
        // (knee, foot, knee, ...) nudging +/- one step forever (real
        // report: bodyAngle bouncing 451.00 <-> 451.25). So the guard stays
        // armed whenever the second-closest contact is also touching and
        // the center of mass lies horizontally between the two.
        float reversalGuardComX = settleMassCenter().x;
        float reversalGuardComOffset = fabsf(pivot.x - reversalGuardComX);
        BOOL reversalGuardArmed = (reversalGuardComOffset <= SIMULATION_ROCKY_REVERSAL_GUARD_MAX_COM_OFFSET);
        if (!reversalGuardArmed && !(legHidden && bodyHidden))
        {
            RockyLegContact supportA, supportB;
            settleToppleContactPoints(&supportA, &supportB);
            if (supportB.clearance <= robotLengthToEnvWorld(SIMULATION_ROCKY_SUPPORT_CONTACT_TOL))
            {
                float supportLo = (supportA.world.x < supportB.world.x) ? supportA.world.x : supportB.world.x;
                float supportHi = (supportA.world.x < supportB.world.x) ? supportB.world.x : supportA.world.x;
                if (reversalGuardComX >= supportLo && reversalGuardComX <= supportHi)
                    reversalGuardArmed = TRUE; // supported on both sides -- a real rest pose, keep anti-oscillation on
            }
        }
        ...
        {
            float pivotComX = settleMassCenter().x;
            int currentPivotSide = (pivot.x < pivotComX) ? -1 : 1;
            if (rockyPivotSide != 0 && currentPivotSide != rockyPivotSide && reversalGuardArmed)
            {
                SIM_LOG("[SETTLE] bodyAngle=%.2f pivot flipped sides of the center of mass (side=%d -> %d) -- halving rockyBodySettleStep to stop it vibrating across center\n",
                       (*bodyAngle), rockyPivotSide, currentPivotSide);
                rockyBodySettleStep *= 0.5f;
            }
            else if (rockyPivotSide != 0 && currentPivotSide != rockyPivotSide)
            {
                SIM_LOG("[SETTLE] bodyAngle=%.2f pivot flipped sides of the center of mass (side=%d -> %d), but the center of mass is %.4f away and not between two contacts -- a real topple, not vibration, so not halving and not reversal-guarding\n",
                       (*bodyAngle), rockyPivotSide, currentPivotSide, reversalGuardComOffset);
        ...
        // Fallback for the exact mirror of Probe 2's "dangling foot" case
        ...
        // directly, Probe 1 is structurally blind to the entire toppling
        ...
        // Probe 2's own !legHidden guard above: with no leg being
        ...
        float baseDanglingClearance = legHidden ? 0.0f : settleDanglingPointClearance();
        float bestClearanceGain = 0.0f;
        ...
        for (int dir = -1; dir <= 1; dir += 2, dirIndex++)
        {
            ...
            PointF shift = rotateActiveBodyAroundPivot(pivot, deltaAngle);
            ...
            // check Probe 2's own dangling-foot fallback uses just above
            // -- see the comment before this loop for why Probe 1 needs
            ...
            // its own !legHidden nested inside).
            if (!robotCollidesWithEnvironment())
            {
                PointF com = settleMassCenter();
                ...
                if (!legHidden)
                {
                    ...
                    // A "dangling point closer" move must not RAISE the center of mass:
                    // gravity never lifts a body just to bring a hovering point down.
                    // Without this, a COM-drop commit one tick and a gain commit back
                    // the other way the next tick bounced bodyAngle +/- one step forever
                    // (real report: 334.39 <-> 335.39, COM rising 0.01465 on every gain step).
                    if (clearance >= 0.0f && comDrop >= -SIMULATION_LEG_SETTLE_MIN_COM_DROP)
                        gainAtDir[dirIndex] = gain;

                    if (clearance >= 0.0f && comDrop >= -SIMULATION_LEG_SETTLE_MIN_COM_DROP && gain > bestClearanceGain)
                    {
                        bestClearanceGain = gain;
                        bestClearanceBodyAngle = (*bodyAngle);
        ...
        {
            SIM_LOG("[SETTLE] bodyAngle=%.2f both directions still improve (drop=%.5f/%.5f comDrop=%.5f/%.5f gain=%.5f/%.5f) -- shrinking instead of picking one, to avoid oscillating across the exact resting angle\n",
                   (*bodyAngle), dropAtDir[0], dropAtDir[1], comDropAtDir[0], comDropAtDir[1], gainAtDir[0], gainAtDir[1]);
            rockyBodySettleStep *= 0.5f;
        }
                 ...
                 && reversalGuardArmed)
        {
            ...
            // from whatever Probe 1 actually committed last tick -- see
            ...
            rockyBodyReversalGuardBlockedThisTick = TRUE;
            SIM_LOG("[SETTLE] bodyAngle=%.2f bestDrop wants to pivot back the OPPOSITE way from last tick's own commit (dir %d -> %d) -- shrinking instead of reversing, to stop it bouncing between two angles forever (blocked streak %d/%d)\n",
                   (*bodyAngle), rockyBodyLastCommitDir, bestDropDir, rockyBodyReversalBlockedStreak, SIMULATION_ROCKY_BODY_REVERSAL_BLOCK_STREAK);
            shrinkRockyBodyStepForReversalGuard();
        }
        else if (bestDrop > SIMULATION_LEG_SETTLE_MIN_DROP)
        {
            ...
            SIM_LOG("[SETTLE] bodyAngle=%.2f pivoted the assembly %.4f deg around its ground contact, it fell %.5f further this tick (of %.5f available)\n",
                   (*bodyAngle), rockyBodySettleStep, appliedDrop, bestDrop);
        }
                 ...
                 && reversalGuardArmed)
        {
            ...
            rockyBodyReversalBlockedStreak++;
            rockyBodyReversalGuardBlockedThisTick = TRUE;
            SIM_LOG("[SETTLE] bodyAngle=%.2f bestComDrop wants to pivot back the OPPOSITE way from last tick's own commit (dir %d -> %d) -- shrinking instead of reversing, to stop it bouncing between two angles forever (blocked streak %d/%d)\n",
                   (*bodyAngle), rockyBodyLastCommitDir, bestComDropDir, rockyBodyReversalBlockedStreak, SIMULATION_ROCKY_BODY_REVERSAL_BLOCK_STREAK);
            shrinkRockyBodyStepForReversalGuard();
        }
        else if (bestComDrop > SIMULATION_LEG_SETTLE_MIN_COM_DROP)
        {
            ...
            SIM_LOG("[SETTLE] bodyAngle=%.2f pivoted the assembly %.4f deg around its ground contact, center of mass dropped %.5f further (no further translation, gravity settling by mass alone)\n",
                   (*bodyAngle), rockyBodySettleStep, bestComDrop);
        }
                 ...
                 && reversalGuardArmed)
        {
            ...
            rockyBodyReversalBlockedStreak++;
            rockyBodyReversalGuardBlockedThisTick = TRUE;
            SIM_LOG("[SETTLE] bodyAngle=%.2f bestClearanceGain wants to pivot back the OPPOSITE way from last tick's own commit (dir %d -> %d) -- shrinking instead of reversing, to stop it bouncing between two angles forever (blocked streak %d/%d)\n",
                   (*bodyAngle), rockyBodyLastCommitDir, bestClearanceGainDir, rockyBodyReversalBlockedStreak, SIMULATION_ROCKY_BODY_REVERSAL_BLOCK_STREAK);
            shrinkRockyBodyStepForReversalGuard();
        }
        else if (bestClearanceGain > SIMULATION_LEG_SETTLE_MIN_CLEARANCE_GAIN)
        {
            ...
            SIM_LOG("[SETTLE] bodyAngle=%.2f pivoted the assembly %.4f deg around its ground contact, dangling point %.5f closer to the ground (no further translation)\n",
                   (*bodyAngle), rockyBodySettleStep, bestClearanceGain);
        }
        else
        {
            ...
            for (int dir = -1; dir <= 1; dir += 2, scoutDirIndex++)
            {
                ...
                PointF scoutShift = rotateActiveBodyAroundPivot(pivot, scoutDelta);
                ...
                if (scoutClear)
                {
                    PointF scoutCom = settleMassCenter();
                    scoutComDrop = baseComY - scoutCom.y;

                    if (!legHidden)
                    {
                        float scoutClearance = settleDanglingPointClearance();
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                else if (scoutClearAtDir[i] && scoutComDropAtDir[i] > SIMULATION_LEG_SETTLE_MIN_COM_DROP)
                {
                    ...
                    value = scoutComDropAtDir[i];
                         ...
                         && scoutComDropAtDir[i] >= -SIMULATION_LEG_SETTLE_MIN_COM_DROP) // same "never raise the COM" rule as the direct gain branch
                {
                    tier = 0;
                 ...
                 && reversalGuardArmed)
            {
                ...
                // back the way Probe 1 just came from last tick, so
                ...
                rockyBodyReversalBlockedStreak++;
                rockyBodyReversalGuardBlockedThisTick = TRUE;
                SIM_LOG("[SETTLE] bodyAngle=%.2f scouted improvement is back the OPPOSITE way from last tick's own commit -- shrinking instead of reversing, same as the direct branches above (blocked streak %d/%d)\n",
                       (*bodyAngle), rockyBodyReversalBlockedStreak, SIMULATION_ROCKY_BODY_REVERSAL_BLOCK_STREAK);
            }
            else if (scoutWinner != -1)
            {
                ...
                rotateActiveBodyAroundPivot(pivot, nudgeDelta); // committed, kept below
                ...
                // Probe 2's own scout branch: the body genuinely isn't
                ...
                SIM_LOG("[SETTLE] bodyAngle=%.2f pivoted around its ground contact, scouted real improvement %.1f deg out (drop=%.5f comDrop=%.5f gain=%.5f), fell %.5f this tick -- nudging that way instead of shrinking\n",
                       (*bodyAngle), dir * SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG, scoutDrop, scoutComDrop, scoutGain, nudgeAppliedDrop);
                scoutedBody = TRUE;
                probe1Committed = TRUE;
                // See bestDrop's own copy of this same check further above
                // for why only a genuine direction CHANGE clears the streak.
            ...
            if (!scoutedBody)
            {
                if (rockyBodyReversalGuardBlockedThisTick) shrinkRockyBodyStepForReversalGuard(); else rockyBodySettleStep *= 0.5f;
            }
        }
    }

    // Stalemate check: Probe 2 and Probe 1 run sequentially in THIS same
    // call, each one committing based on whatever the OTHER one just left
    // behind (Probe 2 above bent the knee using the body angle from
    // Probe 1's PREVIOUS tick; Probe 1 just above pivoted the body using
    // the kneeAngle Probe 2 only just set, this same tick). Neither
    ...
    if (probe2Committed && probe1Committed)
    {
        rockyBothProbesCommittedStreak++;
        if (rockyBothProbesCommittedStreak >= SIMULATION_LEG_SETTLE_STALEMATE_STREAK)
        {
            SIM_LOG("[SETTLE] both probes have committed a move on every one of the last %d ticks -- treating as a stalemate (each one chasing an improvement the other's last move created) and shrinking both step sizes\n",
                   rockyBothProbesCommittedStreak);
            for (int j = 0; j < settleJointCount(); j++) *settleJointStepPtr(j) *= 0.5f;
            rockyBodySettleStep *= 0.5f;
    ...
    if (probe2DirectCommit && !probe1Committed)
    {
        ...
        if (rockyKneeSettleGrowthStreak >= SIMULATION_LEG_SETTLE_GROWTH_STREAK)
        {
            ...
            if (rockyKneeSettleStep < SIMULATION_LEG_SETTLE_STEP_DEG)
            {
                ...
                SIM_LOG("[SETTLE] kneeAngle=%.2f committed a clean improvement on every one of the last %d ticks with the body probe quiet -- growing the step back up from %.4f to %.4f deg to cover ground faster\n",
                       (*settleJointPtr(0)), SIMULATION_LEG_SETTLE_GROWTH_STREAK, rockyKneeSettleStep, grownKneeStep);
                rockyKneeSettleStep = grownKneeStep;
            }
            // Semni's hip joint grows back the same way (no-op for Rocky,
            // which only has the one Probe 2 joint).
            if (settleJointCount() > 1 && semniHipSettleStep < SIMULATION_LEG_SETTLE_STEP_DEG)
            {
                semniHipSettleStep *= 2.0f;
                if (semniHipSettleStep > SIMULATION_LEG_SETTLE_STEP_DEG)
                    semniHipSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
            }
        }
    }
    ...
    if (probe1DirectCommit && !probe2Committed)
    {
        ...
        if (rockyBodySettleGrowthStreak >= SIMULATION_LEG_SETTLE_GROWTH_STREAK)
        {
            ...
            if (rockyBodySettleStep < SIMULATION_BODY_SETTLE_STEP_DEG)
            {
                ...
                SIM_LOG("[SETTLE] bodyAngle=%.2f committed a clean improvement on every one of the last %d ticks with the knee probe quiet -- growing the step back up from %.4f to %.4f deg to cover ground faster\n",
                       (*bodyAngle), SIMULATION_LEG_SETTLE_GROWTH_STREAK, rockyBodySettleStep, grownBodyStep);
    ...
    // Probe 2 is skipped entirely while the leg is hidden (its own
    // !legHidden guard above), which means rockyKneeSettleStep just
    ...
    // no matter how thoroughly Probe 1 alone had already converged: the
    ...
    // moved, long after it had actually finished settling. Treat Probe 2
    // as trivially "already at its floor" when there's no leg for it to
    // ever run against, same as Probe 2's own skip condition, so
    // convergence only ever waits on whichever probe(s) can actually run.
    //
    // The exact same unreachability hits during an E/Q press: Probe 2's
    // OWN top-level guard is "!legHidden && !rockyKneeSettleSuppressed
    ...
    // Probe 2 doesn't run then either, and rockyKneeSettleStep is just as
    ...
    // diagnostic pass found: whenever Probe 2's own top-level guard
    // (further up) is false because the leg's current contact is one
    // Probe 2 must leave alone (ARC/KNEE/FOOT already resting), the
    ...
    // FOOT while Probe 1 ALSO happens to run its own step down to zero
    ...
    if ((legHidden || rockyKneeSettleSuppressed || legRestingOnImmovableJoint || settleAllJointStepsBelow(SIMULATION_LEG_SETTLE_MIN_STEP_DEG))
        && rockyBodySettleStep < SIMULATION_LEG_SETTLE_MIN_STEP_DEG
        && !rockyBodyReversalGuardBlockedThisTick)
    {
        // Diagnostic only, temporary -- before actually latching
        // converged, re-check at a full, un-shrunk scout angle (the same
        // SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG Probe 1's own scout
        // fallback further up already uses for exactly this "is there
        // real improvement further out" question) whether genuine
        // progress is still available in either direction right now.
        // rockyBodySettleStep having shrunk below the floor is NOT the
        // same guarantee as "no more progress exists anywhere" -- it can
        // shrink there purely from repeated directionsStraddle/pivot-
        // flip/reversal-guard halvings during a twisty settle, and the
        // step only ever grows back via a strict run of
        // SIMULATION_LEG_SETTLE_GROWTH_STREAK consecutive CLEAN single-
        // probe commits, which frequent halving events right near the
        // true resting angle can keep preventing from ever completing --
        // even while real, unresolved improvement remains on one side.
        // This measures what a full-size probe finds RIGHT NOW, purely
        // to check that assumption before trusting it, then fully undoes
        // itself either way -- no committed state changes as a result of
        // this block, converged or not.
        //
        // Only runs while View > Simulation Log is on: its whole output is
        // one [SETTLE] log line, and it costs two full 20 deg scout drops
        // (a real report showed convergence ticks taking ~80 ms).
        double perfSanityStart = perfNowMs();
        if (simPhysicsLogEnabled && !(bodyHidden && legHidden))
        {
            PointF sanityPivot = settleTopplePivotWorld();
            float sanityBaseAngle = (*bodyAngle);
            float sanityBaseComY = settleMassCenter().y;
            float sanityDrop[2] = { 0.0f, 0.0f };
            float sanityComDrop[2] = { 0.0f, 0.0f };
            int si = 0;
            for (int sdir = -1; sdir <= 1; sdir += 2, si++)
            {
                float sanityDelta = sdir * SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG;
                PointF sanityShift = rotateActiveBodyAroundPivot(sanityPivot, sanityDelta);
                float sanityResidual = clearPivotRoundingResidual();
                float d = dropActiveRobotToRest();
                if (d > 0.0f)
                    translateActiveRobot(0.0f, d);
                sanityDrop[si] = d;
                if (!robotCollidesWithEnvironment())
                {
                    PointF com = settleMassCenter();
                    sanityComDrop[si] = sanityBaseComY - com.y;
                }
                if (sanityResidual > 0.0f)
                    translateActiveRobot(0.0f, -sanityResidual);
                translateActiveRobot(-sanityShift.x, -sanityShift.y);
                (*bodyAngle) = sanityBaseAngle;
            }
            SIM_LOG("[SETTLE] convergence sanity check -- scouted %.1f deg each way from the current pivot: drop=%.5f/%.5f comDrop=%.5f/%.5f (min thresholds: drop>%.5f comDrop>%.5f)\n",
                   SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG, sanityDrop[0], sanityDrop[1], sanityComDrop[0], sanityComDrop[1],
                   (double)SIMULATION_LEG_SETTLE_MIN_DROP, (double)SIMULATION_LEG_SETTLE_MIN_COM_DROP);
        }
        ...
        // Close any small leftover gap under the second contact point
        // before calling this the rest pose -- see seatRockyDanglingPoint.
        perfLastSettleSanityMs = perfNowMs() - perfSanityStart;

        double perfSeatStart = perfNowMs();
        seatRockyDanglingPoint();
        perfLastSettleSeatMs = perfNowMs() - perfSeatStart;

        rockySettleConverged = TRUE;
        SIM_LOG("[SETTLE] converged -- kneeAngle=%.2f bodyAngle=%.2f, no further improvement found\n",
               (*settleJointPtr(0)), (*bodyAngle));
    }
    else if (rockyBodyReversalGuardBlockedThisTick
             && (legHidden || rockyKneeSettleSuppressed || legRestingOnImmovableJoint || settleAllJointStepsBelow(SIMULATION_LEG_SETTLE_MIN_STEP_DEG))
             && rockyBodySettleStep < SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
    {
        // Every OTHER condition for convergence was true this tick (knee
        // side done, body step at its floor) except this one -- purely so
        // it's visible in the log that convergence was deliberately held
        // off specifically because the reversal guard just set aside a
        // real improvement, not because nothing was found. Without this
        // line the log otherwise looks identical to any other blocked-
        // streak tick, giving no hint that this was the one tick
        // convergence would otherwise have latched on.
        SIM_LOG("[SETTLE] convergence held off -- bodySettleStep is at its floor but the reversal guard just blocked a genuine improvement this tick, so this is not actually the resting pose yet\n");
    }
}
...
static BOOL applyGravityStep(HWND hWnd, float step)
{
    ...
    if (!suppressThisCall)
        SIM_LOG("[GRAVITY] step=%.6f y_before=%.6f y_after_step=%.6f landed=%d\n",
    ...
    if (landed)
    {
        ...
        if (!suppressThisCall)
        {
            SIM_LOG("[GRAVITY]   search: safe=%.6f blocked=%.6f y_final=%.6f\n",
            ...
            // as a flat one.
            float envHalfWidth = simEnvLineHalfWidthWorld();
            SIM_LOG("[GRAVITY]   envHalfWidth=%.5f\n", envHalfWidth);
            ...
            if (app.robotScene.activeKind == ROBOT_KIND_SEMNI)
            {
                ...
                for (int c = 0; c < NUM_ROBOT_BODY_CIRCLES; c++)
                {
                    ...
                    SIM_LOG("[GRAVITY]   circle=%-4s world=(%.5f,%.5f) radius=%.5f dist=%.5f combined=%.5f gap=%.5f\n",
            ...
            else if (app.robotScene.activeKind == ROBOT_KIND_ROCKY)
            {
                ...
                for (int e = 0; e < NUM_ROCKY_RECT_SEGMENTS; e++)
                {
                    ...
                    SIM_LOG("[GRAVITY]   edge=%-6s world=(%.5f,%.5f)-(%.5f,%.5f) dist=%.5f combined=%.5f gap=%.5f\n",
            ...
            else if (app.robotScene.activeKind == ROBOT_KIND_STILO)
            {
                ...
                for (int c = 0; c < NUM_STILO_BODY_CIRCLES; c++)
                {
                    ...
                    float combined = eRadius + envHalfWidth;
                    float dist = nearestEnvDistance(ecx, ecy);
                    SIM_LOG("[GRAVITY]   circle=%-5s world=(%.5f,%.5f) radius=%.5f dist=%.5f combined=%.5f gap=%.5f\n",
                           dbgNames[c], ecx, ecy, eRadius, dist, combined, dist - combined);
                }
            }
        }
        ...
        if (!suppressThisCall)
        {
            ...
            // rockyToppleLastPivotCategory mid-settle and letting Probe 1
            ...
            // event.
            SIM_LOG("[SETTLE] fresh landing detected (landed=FALSE -> TRUE) -- resetting probe step sizes and anti-oscillation state\n");
        ...
        else
        {
            ...
            // [PERF] diagnostic, temporary -- times this whole batch of up
            // to SIMULATION_LEG_SETTLE_FAST_FORWARD_TICKS advanceRockySettle
            // calls, since a real report showed FPS occasionally dropping
            // to ~20 with no obvious cause yet identified. This loop is one
            // of the strongest suspects: each advanceRockySettle call does
            // real collision/distance searches against the WHOLE
            // environment (dropActiveRobotToRest's own binary search,
            // robotCollidesWithEnvironment, rockyToppleContactPoints, the
            // convergence sanity check's own scout probe), and it can run
            // up to 16 times back to back inside ONE WM_TIMER tick -- all
            // of that happens BEFORE this tick's own InvalidateRect, so a
            // slow batch here delays how soon the next real frame can even
            // be scheduled, which reads as a dropped frame rate exactly
            // like a slow render would, even though nothing in
            // renderCombinedFrame itself was slow. See renderCombinedFrame's
            // own [PERF] instrumentation for the render-side half of this
            // same investigation.
            double perfSettleStart = perfNowMs();
            int perfSettleCalls = 0;
            double perfSlowestCallMs = 0.0; // most expensive advanceRockySettle call so far this tick
            double perfSlowestSanityMs = 0.0, perfSlowestSeatMs = 0.0; // that call's convergence extras
            float comYDroppedThisTick = 0.0f; // total settle drop this tick -- see the drop cap below

            for (int ff = 0; ff < SIMULATION_LEG_SETTLE_FAST_FORWARD_TICKS; ff++)
            {
                // Real-time budget cap (SIMULATION_LEG_SETTLE_FAST_FORWARD_
                // BUDGET_MS -- see config.h's own comment for why this was
                // added): only checked from the second call onward, so the
                // very first call of every tick still always happens no
                // matter how starved this tick already is -- same
                // forward-progress guarantee SIMULATION_LEG_SETTLE_FAST_
                // FORWARD_TICKS itself relies on. Any ticks this skips are
                // simply picked back up on the NEXT WM_TIMER tick, since
                // this whole loop just runs again as long as
                // rockySettleConverged is still FALSE.
                //
                // Also stops BEFORE a call that would overshoot the budget,
                // not just after the budget is already gone: one call can
                // itself take longer than the whole budget, so "elapsed <
                // budget" alone let a tick run e.g. 8 ms + one more 15 ms
                // call. Predicts the next call's cost from the slowest call
                // seen so far this tick.
                if (ff > 0)
                {
                    double spentMs = perfNowMs() - perfSettleStart;
                    if (spentMs + perfSlowestCallMs > (double)SIMULATION_LEG_SETTLE_FAST_FORWARD_BUDGET_MS)
                        break; // next call would blow this tick's settle budget -- finish on a later tick
                }

                // Whichever robot is settling (Rocky or Semni) -- these used
                // to read Rocky's fields unconditionally, so for Semni they
                // never changed, this loop never saw "something moved", and
                // it batched up to SIMULATION_LEG_SETTLE_FAST_FORWARD_TICKS
                // settle calls (each allowed to drop the body) into every
                // tick: Semni sank many times faster than Rocky.
                float jointBefore[2] = { 0.0f, 0.0f };
                for (int j = 0; j < settleJointCount(); j++)
                    jointBefore[j] = *settleJointPtr(j);
                float bodyBefore = *settleBodyAnglePtr();
                float comYBefore = settleMassCenter().y;

                double perfCallStart = perfNowMs();
                advanceRockySettle();
                double perfCallMs = perfNowMs() - perfCallStart;
                if (perfCallMs > perfSlowestCallMs)
                {
                    perfSlowestCallMs = perfCallMs;
                    perfSlowestSanityMs = perfLastSettleSanityMs;
                    perfSlowestSeatMs = perfLastSettleSeatMs;
                }
                perfSettleCalls++;
                ...
                for (int j = 0; j < settleJointCount(); j++)
                {
                    float d = fabsf(*settleJointPtr(j) - jointBefore[j]);
                    if (d > kneeDelta) kneeDelta = d;
                }
                float bodyDelta = fabsf(*settleBodyAnglePtr() - bodyBefore);
                ...
                // Also stop once the body has DROPPED a real amount this
                // tick: SIMULATION_LEG_SETTLE_MAX_DROP_PER_TICK caps a single
                // settle call's drop at one gravity step, and batching
                // several such calls would otherwise let a settling robot
                // sink faster than gravity itself moves it.
                comYDroppedThisTick += comYBefore - settleMassCenter().y;
                if (comYDroppedThisTick >= SIMULATION_LEG_SETTLE_MAX_DROP_PER_TICK)
                    break;
            }

            double perfSettleElapsed = perfNowMs() - perfSettleStart;
            if (perfSettleElapsed >= (double)SIMULATION_PERF_WARN_FRAME_MS)
            {
                SIM_FPS_LOG("[PERF] settle fast-forward took %.2fms across %d advanceRockySettle call(s) (avg %.2fms/call, slowest %.2fms [sanityCheck=%.2fms seat=%.2fms probes=%.2fms], budget %dms) -- this ran INSIDE the current WM_TIMER tick, before its own repaint was even scheduled\n",
                       perfSettleElapsed, perfSettleCalls,
                       perfSettleCalls > 0 ? perfSettleElapsed / (double)perfSettleCalls : 0.0,
                       perfSlowestCallMs, perfSlowestSanityMs, perfSlowestSeatMs,
                       perfSlowestCallMs - perfSlowestSanityMs - perfSlowestSeatMs,
                       SIMULATION_LEG_SETTLE_FAST_FORWARD_BUDGET_MS);
            }
        }
    }
...
// Rocky's knee (E/Q), while genuinely HELD: its own dedicated timer,
// completely independent of autoGravityActive/AUTO_GRAVITY_TIMER_ID --
// unlike auto-gravity, this needs to keep rotating the knee smoothly
// whether or not Auto Gravity happens to be on (E/Q already works with
// Auto Gravity off, it just doesn't also settle the rest of the body in
// that case -- see WM_KEYDOWN's own E/Q case). See
// advanceRockyKneeHold's own comment (further down) for the full
// reasoning on why this exists instead of just continuing to rely on
// Windows' own WM_KEYDOWN auto-repeat.
#define ROCKY_KNEE_HOLD_TIMER_ID 1004

// TRUE for as long as E, respectively Q, is physically held down -- set on
// that key's own WM_KEYDOWN (the first, non-auto-repeat press only; see
// isAutoRepeatEQ's own comment at that call site), cleared on its matching
// WM_KEYUP. Never both driven by the SAME variable: E and Q rotate in
// opposite directions, and a user can genuinely hold both at once (net
// rotation cancels to zero, same as it always has), so each needs its own
// independent held state. advanceRockyKneeHold reads both every tick to
// decide which way (if either) to actually rotate.
static BOOL rockyKneeHoldE = FALSE;
static BOOL rockyKneeHoldQ = FALSE;

// Real-time (GetTickCount) timestamp of the last advanceRockyKneeHold
// call -- same "scale by actual elapsed time, not an assumed fixed tick
// length" reasoning as autoGravityLastTickTime's own comment just above,
// applied to the knee-hold driver instead of the gravity-fall one.
static DWORD rockyKneeHoldLastTickTime = 0;
...
// Real-time (GetTickCount) timestamp of the last time E/Q's own settle
// probes got revived back to full step size -- either by a keyup, or by
// WM_KEYDOWN's own periodic re-arm while the key is held (see
// reviveRockyBodyProbeStepSizes and its call sites below). Reset to "now"
// on every fresh (non-auto-repeat) E/Q press so a long-overdue press
// doesn't immediately treat itself as already due for another revive one
// repeat later -- the interval should always be measured from when THIS
// hold actually started.
static DWORD rockyEQHoldLastReviveTick = 0;
...
static void advanceAutoGravity(HWND hWnd)
{
    ...
    // Real-world gravity (SIMULATION_GRAVITY_MPS2, 9.81 m/s^2), converted
    // into robot world units with the same mm-per-unit scale the "Size:
    // W x H mm" readout uses (MM_PER_WORLD_UNIT x the Robot Size slider) --
    // so the robot falls at its real physical speed for its displayed size.
    // Mass deliberately plays no part: without air resistance every object
    // falls the same way regardless of weight (weight still feeds the
    // exported masses and the mass-center position). 1 m/s^2 = 0.001
    // mm/ms^2, and 1 m/s = 1 mm/ms.
    float mmPerUnit = MM_PER_WORLD_UNIT * graphicsGetRobotScale();
    if (mmPerUnit <= 0.0f) mmPerUnit = MM_PER_WORLD_UNIT;
    float accelUnitsPerMs2 = (SIMULATION_GRAVITY_MPS2 * 0.001f) / mmPerUnit;
    float maxVelocity = SIMULATION_AUTO_GRAVITY_MAX_VELOCITY_MPS / mmPerUnit;

    autoGravityVelocity += accelUnitsPerMs2 * simElapsed;
...
static void advancePostRotateSettle(HWND hWnd)
{
    ...
    // either way.
    BOOL landed = applyGravityStep(hWnd, SIMULATION_GRAVITY_STEP); // weight no longer changes fall speed -- see advanceAutoGravity
    ...
    BOOL atRest = landed && (!settleSupportedKind() || rockySettleConverged);
...
// Revives Probe 2/Probe 1's own step sizes (and the growth-streak/both-
// probes-committed bookkeeping tied to them) back to their full starting
// size, and re-arms rockySettleConverged/postRotateSettleActive/the timer
// so the settle loop actually starts probing again -- WITHOUT touching
// Probe 1's own direction/identity memory (rockyPivotSide,
// rockyBodyLastCommitDir, rockyToppleLastPivotCategory,
// rockyBodyReversalBlockedStreak). See E/Q's own WM_KEYUP case for the
// original reasoning on why this exact split (revive step size, preserve
// identity) is correct -- that reasoning is unchanged, this just factors
// the actual state change out into one shared place instead of a second,
// driftable copy. Also called periodically WHILE E/Q is held (WM_KEYDOWN's
// own auto-repeat case, throttled by rockyEQHoldLastReviveTick), not just
// once on release -- a real report showed a long hold visibly lagging
// behind a run of individual taps, traced to exactly this: a tap's own
// keyup already re-primes the settle probes at full strength after every
// single nudge, but a hold only ever got that same priming once, whenever
// it finally released, so the whole hold's worth of kneeAngle movement was
// chased by whatever step size had already shrunk down from BEFORE the
// hold began. Periodic re-arming during the hold gives it the same
// repeated fresh-attempt priming a run of taps already got for free.
static void reviveRockyBodyProbeStepSizes(HWND hWnd)
{
    rockySettleConverged = FALSE;
    rockyKneeSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
    semniHipSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
...
// Rotates Rocky's kneeAngle smoothly, by however much real time has
// actually passed since the last call, for as long as E and/or Q is
// genuinely held down -- same "scale by real elapsed time instead of
// assuming a fixed tick length" fix advanceAutoGravity already uses for
// the gravity fall (see its own comment), applied here to fix a different
// but related report: "holding E or Q feels jerky/inconsistent, not
// smooth like the fall does." Before this, a held E/Q relied entirely on
// Windows' own WM_KEYDOWN auto-repeat to keep nudging kneeAngle by a fixed
// SIMULATION_JOINT_ROTATE_STEP_DEG per repeated message -- auto-repeat has
// its own OS-level initial delay before it starts (read as a stutter/pause
// right at the start of a hold), then fires at a fixed rate set in the
// user's own keyboard Control Panel settings, unrelated to the screen's
// actual refresh rate -- so the rotation speed while held could feel
// noticeably chunky or uneven depending purely on that OS setting, instead
// of the smooth, consistent motion a real-time-driven rate gives
// regardless of it. Called every ROCKY_KNEE_HOLD_TIMER_ID tick (WM_TIMER
// below) for as long as either key is held; WM_KEYDOWN's own E/Q case
// still applies exactly one discrete SIMULATION_JOINT_ROTATE_STEP_DEG nudge
// on the initial (non-auto-repeat) press only, so a plain tap's feel is
// completely unchanged -- this only ever runs for a genuine hold.
static void advanceRockyKneeHold(HWND hWnd)
{
    if (appMode != APP_MODE_SIMULATION || app.robotScene.activeKind != ROBOT_KIND_ROCKY) return;
    if (app.robotScene.rocky.legHidden) return; // same no-op-on-legless-Rocky rule as WM_KEYDOWN's own E/Q case
    if (!rockyKneeHoldE && !rockyKneeHoldQ) return; // nothing actually held -- WM_TIMER's own dispatcher stops this timer shortly after this, see its comment

    DWORD now = GetTickCount();
    DWORD elapsed = now - rockyKneeHoldLastTickTime;
    if (elapsed == 0) { InvalidateRect(hWnd, NULL, FALSE); return; } // already advanced this exact millisecond

    if (elapsed > SIMULATION_JOINT_ROTATE_HOLD_MAX_DT_MS)
        elapsed = SIMULATION_JOINT_ROTATE_HOLD_MAX_DT_MS; // avoid a big catch-up jump after a stall

    rockyKneeHoldLastTickTime = now;

    // Same clockwise-is-negative convention WM_KEYDOWN's own single-tap
    // case uses (rotatePoint's CCW-for-positive-angle matrix) -- Q
    // (counter-clockwise) is positive, E (clockwise) is negative. Both
    // held at once cancels to zero, same net-nothing result a real report
    // never actually asked to be treated any differently.
    int dir = (rockyKneeHoldQ ? 1 : 0) + (rockyKneeHoldE ? -1 : 0);
    if (dir != 0)
    {
        Rocky* r = &app.robotScene.rocky;
        r->kneeAngle += (float)dir * SIMULATION_JOINT_ROTATE_RATE_DEG_PER_MS * (float)elapsed;

        resolveUpwardIfPenetrating(hWnd, SIMULATION_SLOPE_CORRECTION_MAX);
    }

    // Same periodic settle re-arm WM_KEYDOWN's own E/Q auto-repeat case
    // used to drive on its own -- now driven from here instead, once per
    // real ROCKY_KNEE_HOLD_TIMER_ID tick, which is actually a MORE
    // reliable cadence than however often Windows happened to re-deliver
    // an auto-repeated WM_KEYDOWN. See reviveRockyBodyProbeStepSizes' own
    // comment for the full reasoning on why this revive needs to happen
    // periodically during a hold at all, not just once on release.
    if (!app.robotScene.rocky.legHidden && autoGravityActive
        && (now - rockyEQHoldLastReviveTick) >= SIMULATION_ROCKY_EQ_HOLD_REVIVE_INTERVAL_MS)
    {
        rockyEQHoldLastReviveTick = now;
        SIM_LOG("[SETTLE] E/Q hold periodic re-arm -- reviving knee+body probe step sizes mid-hold, same as a keyup, so a long hold doesn't spend its whole duration chasing an increasingly stale settle attempt\n");
        reviveRockyBodyProbeStepSizes(hWnd);
    }

    // Same triplet WM_KEYDOWN's own E/Q auto-repeat case used to call --
    // keeps Auto Gravity (and, if separately on, Walk) advancing every
    // tick of the hold, not just once it's released. advancePostRotateSettle
    // is a no-op whenever autoGravityActive is on (its own guard), so
    // calling all three unconditionally here is exactly as safe as it was
    ...
    // Same forced-synchronous-repaint fix WM_KEYDOWN's own E/Q case uses,
    // same reason -- WM_TIMER is exactly as low-priority as WM_PAINT, so
    // without this a run of back-to-back ROCKY_KNEE_HOLD_TIMER_ID ticks
    // could each move the robot without any of them actually being drawn
    // until the queue goes quiet.
    UpdateWindow(hWnd);
}

// ---- Semni joint control in Simulation (Q/E = hip, Shift+Q/E = knee) ----
//
// Same hold-to-rotate model as Rocky's E/Q knee (advanceRockyKneeHold
// above): the first press rotates one SIMULATION_JOINT_ROTATE_STEP_DEG step
// immediately, then its own timer keeps rotating at
// SIMULATION_JOINT_ROTATE_RATE_DEG_PER_MS for as long as the key is held
// (Windows' auto-repeat only re-arms the held flag). Q = +angle, E = -angle,
// matching Rocky. Shift is read LIVE on every tick, so pressing or
// releasing Shift mid-hold switches between hip and knee without having to
// let go of Q/E. Rotating a joint pushes the limb into the ground only
// upward (resolveUpwardIfPenetrating), and auto gravity (if on) keeps
// running during the hold, same as Rocky -- Semni has no rotational settle
// (toppling) of its own yet, so it only ever falls straight down.
#define SEMNI_JOINT_HOLD_TIMER_ID 1005

static BOOL semniJointHoldE = FALSE;
static BOOL semniJointHoldQ = FALSE;
static DWORD semniJointHoldLastTickTime = 0;

// Rotates Semni's hip (Shift up) or knee (Shift down) by deltaDeg, then
// lifts the robot out of the ground if that pushed a limb into it.
static void rotateSemniJoint(HWND hWnd, float deltaDeg)
{
    Semni* s = &app.robotScene.robot;
    if (GetAsyncKeyState(VK_SHIFT) & 0x8000)
        s->kneeAngle += deltaDeg;
    else
        s->hipAngle += deltaDeg;
...
static void advanceSemniJointHold(HWND hWnd)
{
    if (appMode != APP_MODE_SIMULATION || app.robotScene.activeKind != ROBOT_KIND_SEMNI) return;
    if (!semniJointHoldE && !semniJointHoldQ) return;

    DWORD now = GetTickCount();
    DWORD elapsed = now - semniJointHoldLastTickTime;
    if (elapsed == 0) { InvalidateRect(hWnd, NULL, FALSE); return; }
    if (elapsed > SIMULATION_JOINT_ROTATE_HOLD_MAX_DT_MS)
        elapsed = SIMULATION_JOINT_ROTATE_HOLD_MAX_DT_MS; // avoid a big catch-up jump after a stall
    semniJointHoldLastTickTime = now;

    int dir = (semniJointHoldQ ? 1 : 0) + (semniJointHoldE ? -1 : 0); // both held cancel out
    if (dir != 0)
        rotateSemniJoint(hWnd, (float)dir * SIMULATION_JOINT_ROTATE_RATE_DEG_PER_MS * (float)elapsed);

    // Same periodic settle re-arm as Rocky's E/Q hold (advanceRockyKneeHold),
    // so Probe 2/2 keep reacting to the pose while a joint is being driven.
    if (autoGravityActive && (now - rockyEQHoldLastReviveTick) >= SIMULATION_ROCKY_EQ_HOLD_REVIVE_INTERVAL_MS)
    {
        rockyEQHoldLastReviveTick = now;
        reviveRockyBodyProbeStepSizes(hWnd);
    }
...
// Real-time FPS counter (View > Show FPS, ID_TOGGLE_SHOW_FPS) -- the
// number canvasRenderFrame draws, top-right, above the Probe 2 Settle
// button. fpsFrameCount is incremented once per call to canvasRenderFrame
// (which itself runs exactly once per renderCombinedFrame call -- see its
// own two call sites -- and every renderCombinedFrame ends in a real
// SwapBuffers, so this is a genuine "frames actually presented" count, not
// an assumed fixed rate). Averaged over a rolling real-time window
// (FPS_UPDATE_INTERVAL_MS) rather than recomputed every single frame -- an
// instantaneous per-frame value (1000 / this-frame's-own-elapsed-ms)
// bounces around far too fast to actually read, especially since this app
// only repaints on demand (a timer tick, a mouse move, a key press) rather
// than on a fixed render loop, so consecutive frames can be an arbitrary
// number of milliseconds apart. Updated (and drawn) unconditionally,
// regardless of showFpsOverlay -- this keeps measuring in the background
// even while the overlay is switched off, so the moment it's switched back
// on it already has a real, already-averaged number instead of starting
// over from a cold 0.
static int fpsFrameCount = 0;
static DWORD fpsWindowStartTick = 0;
...
void canvasRenderFrame(float dimAmount)
{
    ...
    // FPS measurement -- see fpsDisplayValue's own comment above for the
    // full reasoning. Counted here, unconditionally, right at the top of
    // the one function that's guaranteed to run exactly once per actual
    // presented frame -- see that comment for why this specific spot.
    {
        DWORD fpsNow = GetTickCount();
        if (fpsWindowStartTick == 0)
            fpsWindowStartTick = fpsNow; // first frame ever -- start the window from here, not from tick 0

        fpsFrameCount++;

        DWORD fpsElapsed = fpsNow - fpsWindowStartTick;
        if (fpsElapsed >= FPS_UPDATE_INTERVAL_MS)
        {
            fpsDisplayValue = (float)fpsFrameCount * 1000.0f / (float)fpsElapsed;
            fpsFrameCount = 0;
            fpsWindowStartTick = fpsNow;

            // View > FPS Log (Simulation only) -- one line per measurement
            // window (FPS_UPDATE_INTERVAL_MS).
            SIM_FPS_LOG("[FPS] %.1f fps\n", (double)fpsDisplayValue);
        }
    }
    ...
    // same top-right corner Simulation's own Probe 2 toggle button now
    ...
    // FPS counter (View > Show FPS, ID_TOGGLE_SHOW_FPS) -- top-right,
    // directly above the Probe 2 Settle button (hProbe2ToggleBtn, WM_CREATE/
    // WM_SIZE below reserve FPS_OVERLAY_RESERVED_HEIGHT_PX of space above it
    // specifically for this). Left-aligned to that same button's own left
    // edge (glWindowWidth - PROBE2_TOGGLE_MARGIN_PX - PROBE2_TOGGLE_WIDTH_PX)
    // rather than right-aligned to the window edge -- this bitmap-font setup
    // has no cheap way to measure a string's rendered width up front to
    // right-align it, and lining up with the button underneath reads just
    // as tidy without needing that.
    //
    // Same "always full opacity" reasoning as the Mode indicator just
    // above -- this reflects real-time engine performance, not ArcSpline-
    // specific content, so it stays fully readable even while this canvas
    // is the dimmed/inactive subsystem.
    // Simulation only -- not shown in Design > Environment or Design > Robot.
    if (showFpsOverlay && appMode == APP_MODE_SIMULATION)
    {
        char fpsStr[32];
        int fpsRounded = (int)(fpsDisplayValue + 0.5f); // wsprintfA has no float specifier -- see ROTATION's own comment below for the same fix
        wsprintfA(fpsStr, "FPS: %d", fpsRounded);

        // Green/yellow/red at rough smoothness thresholds -- 50+ reads as
        // comfortably smooth, under 30 as visibly choppy, in between as
        // "keep an eye on it". Purely a visual aid, no other effect.
        float fr, fg, fb;
        if (fpsRounded >= 50)      { fr = 0.1f;  fg = 0.55f; fb = 0.15f; } // green
        else if (fpsRounded >= 30) { fr = 0.75f; fg = 0.55f; fb = 0.0f;  } // amber
        else                       { fr = 0.8f;  fg = 0.15f; fb = 0.1f;  } // red

        glColor4f(fr, fg, fb, 1.0f);
        glRasterPos2i(glWindowWidth - PROBE2_TOGGLE_MARGIN_PX - PROBE2_TOGGLE_WIDTH_PX, glWindowHeight - 24);
        glPushAttrib(GL_LIST_BIT);
        glListBase(fontBase - 32);
        glCallLists((GLsizei)strlen(fpsStr), GL_UNSIGNED_BYTE, fpsStr);
        glPopAttrib();
    }
...
void renderCombinedFrame(void)
{
    ...
    // [PERF] diagnostic, temporary -- see this function's own printf at the
    // bottom for the full reasoning (same investigation as applyGravityStep's
    // settle fast-forward timing). perfRobotMs/perfCanvasMs/perfDotsMs are
    // each just the elapsed time around that one call, summed across
    // whichever of the two branches below actually runs (only one does).
    double perfFrameStart = perfNowMs();
    double perfRobotMs = 0.0, perfCanvasMs = 0.0, perfDotsMs = 0.0;

    if (semniActive)
    {
        // ArcSpline underneath...
        double perfT0 = perfNowMs();
        canvasRenderFrame(canvasDimAmount);
        perfCanvasMs = perfNowMs() - perfT0;
    ...
    else
    {
        ...
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        double perfT1 = perfNowMs();
        drawSimulationLiveContactDots();
        perfDotsMs = perfNowMs() - perfT1;
    }

    // SwapBuffers itself is timed separately from the draw calls above --
    // with vsync on, this is where the driver actually BLOCKS waiting for
    // the display's next refresh, which is a completely different kind of
    // cost (GPU/driver wait, not CPU work) from everything measured above
    // it. A [PERF] line dominated by this instead of robot/canvas/dots
    // would point at vsync/driver/GPU load rather than anything in this
    // app's own per-frame logic.
    double perfSwapStart = perfNowMs();
    SwapBuffers(hDC);
    double perfSwapMs = perfNowMs() - perfSwapStart;

    // [PERF] diagnostic, temporary (by explicit request, to chase down a
    // real report of FPS occasionally dropping to ~20 with no cause yet
    // identified) -- only prints once total frame time crosses
    // SIMULATION_PERF_WARN_FRAME_MS (config.h), so a normal fast frame
    // produces no output at all. Read alongside applyGravityStep's own
    // [PERF] settle fast-forward line: THAT one accounts for time spent
    // BEFORE this frame was even scheduled (inside a WM_TIMER tick's own
    // settle work), while this one accounts for time spent actually
    // drawing it -- together they cover both places a frame can go slow.
    // perfRobotMs/perfCanvasMs/perfDotsMs/perfSwapMs won't always sum to
    // exactly the total (glClear, graphicsOnResize, and the various GL
    // state calls between them aren't separately timed), but they cover
    // the expensive parts.
    double perfFrameElapsed = perfNowMs() - perfFrameStart;
    if (perfFrameElapsed >= (double)SIMULATION_PERF_WARN_FRAME_MS)
    {
        SIM_FPS_LOG("[PERF] slow frame: total=%.2fms robot=%.2fms canvas=%.2fms contactDots=%.2fms swapBuffers=%.2fms\n",
               perfFrameElapsed, perfRobotMs, perfCanvasMs, perfDotsMs, perfSwapMs);
    }
}
...
LRESULT CALLBACK WndProcGL(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        hSlowMotionBtn = CreateWindowEx(0, L"BUTTON", L"Slow Motion",
                            ...
                            GetModuleHandle(NULL), NULL);
        SendMessage(hSlowMotionBtn, BM_SETCHECK, BST_CHECKED, 0); // on by default, matching simTimeScale's initial value

        // "Probe 2 Settle" toggle button (top-right) removed by request --
        // same approach as the Walk button just below: hProbe2ToggleBtn is
        // left NULL (never created). Every other use of it in this file is
        // already guarded by "if (hProbe2ToggleBtn)", and without the
        // button ID_PROBE2_TOGGLE can never fire, so rockyProbe2Enabled
        // simply stays at its default TRUE -- Probe 2 (knee settle) keeps
    ...
    case WM_TIMER:
    {
        ...
        // Rocky's knee (E/Q) while genuinely held -- see
        // advanceRockyKneeHold's own comment for the full reasoning. Same
        // defensive shutoff as AUTO_GRAVITY_TIMER_ID just above (Simulation
        // left some other way while this was still running), PLUS its own
        // "nothing's actually held anymore" shutoff -- WM_KEYUP is the
        // normal way this timer gets killed, but if for any reason it's
        // still running with both rockyKneeHoldE/Q now FALSE (a missed or
        // out-of-order keyup, e.g. focus lost mid-hold -- the same
        // structural gap rotateKeyHeld already has, nothing new introduced
        // here), this notices and stops itself within one tick instead of
        // spinning forever on a no-op.
        // Semni's Q/E / Shift+Q/E joint hold -- see advanceSemniJointHold.
        // Stops itself if Simulation was left, the robot kind changed, or
        // both keys are somehow no longer held (e.g. a missed WM_KEYUP).
        if (wParam == SEMNI_JOINT_HOLD_TIMER_ID)
        {
            if (appMode == APP_MODE_SIMULATION && app.robotScene.activeKind == ROBOT_KIND_SEMNI
                && (semniJointHoldE || semniJointHoldQ))
            {
                advanceSemniJointHold(hWnd);
            }
            else
            {
                semniJointHoldE = FALSE;
                semniJointHoldQ = FALSE;
                KillTimer(hWnd, SEMNI_JOINT_HOLD_TIMER_ID);
            }
            return 0;
        }

        if (wParam == ROCKY_KNEE_HOLD_TIMER_ID)
        {
            if (appMode == APP_MODE_SIMULATION && (rockyKneeHoldE || rockyKneeHoldQ))
            {
                advanceRockyKneeHold(hWnd);
            }
            else
            {
                rockyKneeHoldE = FALSE;
                rockyKneeHoldQ = FALSE;
                KillTimer(hWnd, ROCKY_KNEE_HOLD_TIMER_ID);
            }
            return 0;
        }
        ...
        if (wParam == UI_HOTZONE_TIMER_ID && hWndUI)
        {
            ...
            // (where Probe 2's toggle button now happens to live) still
    ...
    case WM_SIZE:
    {
        ...
        // Keep hProbe2ToggleBtn pinned to the top-right corner as the
        ...
        if (hProbe2ToggleBtn)
        {
            SetWindowPos(hProbe2ToggleBtn, NULL,
                         glWindowWidth - PROBE2_TOGGLE_MARGIN_PX - PROBE2_TOGGLE_WIDTH_PX,
                         PROBE2_TOGGLE_MARGIN_PX + FPS_OVERLAY_RESERVED_HEIGHT_PX,
        ...
        }
    {
        ...
        {
            // Same notch for every weight -- mass doesn't change how fast
            // things fall (see advanceAutoGravity).
        ...
        if ((wParam == 'E' || wParam == 'Q') && appMode == APP_MODE_SIMULATION && app.robotScene.activeKind == ROBOT_KIND_ROCKY)
        {
            ...
            // on Probe 2), then unconditionally call dropActiveRobotToRest()
            ...
            // robot.
            // Only the INITIAL press (not a Windows auto-repeat) applies a
            // discrete SIMULATION_JOINT_ROTATE_STEP_DEG nudge here -- a
            // plain tap's feel is completely unchanged from before. A
            // genuine hold's continuous rotation is now entirely
            // ROCKY_KNEE_HOLD_TIMER_ID/advanceRockyKneeHold's job instead
            // (started just below): that driver scales kneeAngle by real
            // elapsed time every tick, which is what actually fixed the
            // "holding feels jerky/chunky" report -- see its own comment
            // for the full reasoning on why relying on the OS's own
            // auto-repeat cadence for a smooth hold never worked well.
            // Auto-repeat events are otherwise ignored entirely here now
            // (no kneeAngle change, no advance* calls, no re-arm) since
            // the dedicated timer already owns all of that for as long as
            // the key stays down.
            BOOL isAutoRepeatEQ = (lParam & 0x40000000) != 0;
            if (isAutoRepeatEQ)
            {
                // Defensive only -- should already be TRUE from this key's
                // own initial press just below, but costs nothing to make
                // sure, in case an initial press was ever somehow missed
                // (e.g. focus changed mid-press) and only repeats arrived.
                if (wParam == 'E') rockyKneeHoldE = TRUE; else rockyKneeHoldQ = TRUE;
                return 0;
            }
            ...
            // Starts (or, if somehow already running, just re-confirms)
            // the dedicated real-time-driven knee-hold timer -- see its
            // own declaration and advanceRockyKneeHold's comment. Both
            // this key's own held flag AND its hold-clock/revive-clock
            // timestamps are (re)initialized to right now on every fresh
            // press, not just the very first one ever -- otherwise a
            // second E press long after the first would see a huge
            // elapsed-time gap on its very first advanceRockyKneeHold tick
            // and try to fold all of that into one jump (clamped by
            // SIMULATION_JOINT_ROTATE_HOLD_MAX_DT_MS, but still not the
            // clean "start of a hold" it should read as).
            if (wParam == 'E') rockyKneeHoldE = TRUE; else rockyKneeHoldQ = TRUE;
            rockyKneeHoldLastTickTime = GetTickCount();
            rockyEQHoldLastReviveTick = rockyKneeHoldLastTickTime;
            SetTimer(hWnd, ROCKY_KNEE_HOLD_TIMER_ID, SIMULATION_AUTO_GRAVITY_INTERVAL_MS, NULL);

            // Same "let the initial press react immediately rather than
            // waiting for the first timer tick" reasoning this block
            // always had -- a single tap should still feel instant.
            advanceAutoGravity(hWnd);
            advancePostRotateSettle(hWnd);
            advanceGait(hWnd);
        ...
        // Semni: Q/E rotates the hip, Shift+Q/Shift+E rotates the knee --
        // see advanceSemniJointHold. wParam is still 'Q'/'E' with Shift
        // held (Shift doesn't change the virtual-key code).
        if ((wParam == 'E' || wParam == 'Q') && appMode == APP_MODE_SIMULATION && app.robotScene.activeKind == ROBOT_KIND_SEMNI)
        {
            BOOL isAutoRepeat = (lParam & 0x40000000) != 0;
            if (wParam == 'E') semniJointHoldE = TRUE; else semniJointHoldQ = TRUE;
            if (isAutoRepeat)
                return 0; // the hold timer is already rotating -- just keep the flag armed

            float step = (wParam == 'E') ? -SIMULATION_JOINT_ROTATE_STEP_DEG : SIMULATION_JOINT_ROTATE_STEP_DEG;
            rotateSemniJoint(hWnd, step);

            semniJointHoldLastTickTime = GetTickCount();
            rockyEQHoldLastReviveTick = semniJointHoldLastTickTime;
            SetTimer(hWnd, SEMNI_JOINT_HOLD_TIMER_ID, SIMULATION_AUTO_GRAVITY_INTERVAL_MS, NULL);
        ...
        if (wParam == 'C' && appMode == APP_MODE_SIMULATION)
        {
            ...
            SIM_LOG("[CONTACT] overlay toggled %s (activeKind=%d)\n",
    ...
    case WM_KEYUP:
    {
        ...
        if ((wParam == VK_LEFT || wParam == VK_RIGHT) && appMode == APP_MODE_SIMULATION)
        {
            ...
            // no leg (Probe 2 skipped, Probe 1 alone driving convergence,
            ...
            if (!(app.robotScene.activeKind == ROBOT_KIND_ROCKY
                ...
                && autoGravityActive)
            {
                SIM_LOG("[SETTLE] VK_LEFT/RIGHT keyup reset fired -- resetting probe step sizes and anti-oscillation state\n");
                rockySettleConverged = FALSE;
                rockyKneeSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
                semniHipSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
                rockyBodySettleStep = SIMULATION_BODY_SETTLE_STEP_DEG;
                ...
                rockyToppleLastPivotCategory = 0;
                rockyBodyReversalBlockedStreak = 0;
                rockyKneeSettleSuppressed = FALSE; // whole-body rotate: nothing else drives kneeAngle, Probe 2 is safe here
        ...
        // Probe 2 (knee-bending) suppressed for the whole settle session
        ...
        // SIMULATION_LEG_SETTLE_MIN_STEP_DEG, but Probe 2's own step-
        ...
        // permanent deadlock: after just one E/Q tap, Probe 2 stays dead
        // for the rest of the session, and if closing some residual gap
        // genuinely needed knee bending (not just the body rotate Probe 1
        ...
        if ((wParam == 'E' || wParam == 'Q') && appMode == APP_MODE_SIMULATION && app.robotScene.activeKind == ROBOT_KIND_ROCKY)
        {
            // This key is no longer held -- stop advanceRockyKneeHold from
            // continuing to rotate on its behalf. Only kill the dedicated
            // timer once NEITHER key is held anymore (the other one may
            // still be down, e.g. releasing E while Q is still held should
            // keep Q's own rotation going uninterrupted).
            if (wParam == 'E') rockyKneeHoldE = FALSE; else rockyKneeHoldQ = FALSE;
            if (!rockyKneeHoldE && !rockyKneeHoldQ)
                KillTimer(hWnd, ROCKY_KNEE_HOLD_TIMER_ID);
            ...
            if (!app.robotScene.rocky.legHidden && autoGravityActive)
            {
                ...
                // settle state on every E/Q release, including Probe 1's
                ...
                SIM_LOG("[SETTLE] E/Q keyup reset fired -- reviving knee+body probe step sizes, leaving body pivot direction/identity memory untouched\n");
                reviveRockyBodyProbeStepSizes(hWnd);
            }
        }
        if ((wParam == 'E' || wParam == 'Q') && appMode == APP_MODE_SIMULATION && app.robotScene.activeKind == ROBOT_KIND_SEMNI)
        {
            if (wParam == 'E') semniJointHoldE = FALSE; else semniJointHoldQ = FALSE;
            if (!semniJointHoldE && !semniJointHoldQ)
                KillTimer(hWnd, SEMNI_JOINT_HOLD_TIMER_ID);

            // Same as Rocky's E/Q keyup: let Probe 2/2 settle the new pose
            // (only with Auto Gravity on, like Rocky).
            if (autoGravityActive)
            {
                SIM_LOG("[SETTLE] Semni Q/E keyup reset fired -- reviving knee/hip + body probe step sizes\n");
                reviveRockyBodyProbeStepSizes(hWnd);
            }
        }
        return 0;
    ...
    case WM_LBUTTONUP:
        ...
        if (app.draggingRobotSim)
        {
            ...
            // zero times, which meant advanceRockySettle (Probe 2/Probe 1,
            ...
            // Probe 2/Probe 1 at all -- that's the entire "home position,
            ...
            if (robotCollidesWithEnvironment() && autoGravityActive)
            {
                ...
                // now, so Probe 2 is safe to run, same as VK_LEFT/RIGHT.
                ...
                SIM_LOG("[SETTLE] WM_LBUTTONUP drag-release reset fired -- resetting probe step sizes and anti-oscillation state\n");
                rockySettleConverged = FALSE;
                rockyKneeSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
                semniHipSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
                rockyBodySettleStep = SIMULATION_BODY_SETTLE_STEP_DEG;
	...
	case WM_COMMAND:
	{
	    ...
	    else if (LOWORD(wParam) == ID_PROBE2_TOGGLE)
	    {
	        ...
	        // effect immediately, no other state to reset (Probe 2 has no
	        // ongoing timer/animation of its own the way Slow Motion/Walk do).
	        rockyProbe2Enabled = (SendMessage(hProbe2ToggleBtn, BM_GETCHECK, 0, 0) == BST_CHECKED);
	    ...
	    else if (LOWORD(wParam) == ID_LAYER_ROBOT || LOWORD(wParam) == ID_LAYER_ENVIRONMENT || LOWORD(wParam) == ID_MODE_SIMULATION)
	    {
	        ...
	        if (LOWORD(wParam) == ID_MODE_SIMULATION)
	        {
	            ...
	            if (robotCollidesWithEnvironment() && autoGravityActive)
	            {
	                SIM_LOG("[SETTLE] Simulation-mode-entry reset fired -- resetting probe step sizes and anti-oscillation state\n");
	                rockySettleConverged = FALSE;
	                rockyKneeSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
	                semniHipSettleStep = SIMULATION_LEG_SETTLE_STEP_DEG;
	                rockyBodySettleStep = SIMULATION_BODY_SETTLE_STEP_DEG;
	            ...
	            SIM_LOG("[ENV] %d segment(s) traced:\n", canvas.segmentResultCount);
	            for (int s = 0; s < canvas.segmentResultCount; s++)
	            {
	                int start = segmentStarts[s];
	                int count = segmentCounts[s];
	                ...
	                SIM_LOG("[ENV]   seg=%d start=(%.5f,%.5f) end=(%.5f,%.5f) thickness=%.5f\n",
	        ...
	        // View > Simulation Log / FPS Log only exist in Simulation --
	        // enabled here on entry, greyed out again on leaving. Their
	        // checkmarks/flags are left as they were, so logging resumes
	        // automatically the next time Simulation is entered.
	        {
	            HMENU hViewMenuForLogs = GetSubMenu(hMenuBar, 2);
	        ...
	        // (by request), so leaving Simulation resets it back to that
	        // default -- checked, SIMULATION_SLOW_MOTION_SCALE -- and every
	        // Simulation session starts in slow motion.
	        if (hSlowMotionBtn)
	        {
	            if (appMode == APP_MODE_SIMULATION)
	            {
	            ...
	            else
	            {
	                ...
	                SendMessage(hSlowMotionBtn, BM_SETCHECK, BST_CHECKED, 0);
	                simTimeScale = SIMULATION_SLOW_MOTION_SCALE;
	            }
	        }
	        ...
	        // Probe 2 toggle -- same show/hide-on-mode-switch treatment as
	        // Slow Motion above. Leaving Simulation also resets it back to
	        // checked/enabled (rockyProbe2Enabled = TRUE) rather than leaving
	        ...
	        if (hProbe2ToggleBtn)
	        {
	            if (appMode == APP_MODE_SIMULATION)
	            {
	                ShowWindow(hProbe2ToggleBtn, SW_SHOW);
	            }
	            else
	            {
	                ShowWindow(hProbe2ToggleBtn, SW_HIDE);
	                SendMessage(hProbe2ToggleBtn, BM_SETCHECK, BST_CHECKED, 0);
	                rockyProbe2Enabled = TRUE;
	            }
	        }
	        ...
	        // Same reasoning again, for the knee-hold timer -- a switch away
	        // from Simulation mid-hold (e.g. the menu, not a keyup) would
	        // otherwise leave ROCKY_KNEE_HOLD_TIMER_ID running and
	        // rockyKneeHoldE/Q still TRUE, ready to start rotating a knee
	        // that isn't even on screen anymore the moment Simulation is
	        // re-entered and E/Q happens to still be physically held down.
	        if (appMode != APP_MODE_SIMULATION && (semniJointHoldE || semniJointHoldQ))
	        {
	            semniJointHoldE = FALSE;
	            semniJointHoldQ = FALSE;
	            KillTimer(hWnd, SEMNI_JOINT_HOLD_TIMER_ID);
	    ...
	    else if (LOWORD(wParam) == ID_SAVE)
	    {
	        ...
	        // Confirm a Weight value that's typed but not yet confirmed
	        // (Enter / click-away) so Save always writes what's in the box --
	        // for every robot kind, not just Rocky.
	        commitWeightEdit(&app);
	    ...
	    else if (LOWORD(wParam) == ID_TOGGLE_SHOW_FPS)
	    {
	        // View > Show FPS -- same independent on/off toggle pattern as
	        // Hide Inactive Layer just above (own small handler, menu item
	        // IS the state, checkmark re-synced after flipping). Doesn't
	        // reset fpsFrameCount/fpsWindowStartTick -- canvasRenderFrame's
	        // own FPS measurement keeps running in the background
	        // unconditionally regardless of this flag (see fpsDisplayValue's
	        // own comment), so switching the overlay back on shows an
	        // already-averaged number immediately instead of starting over.
	        showFpsOverlay = !showFpsOverlay;

	        HMENU hMenuBar = GetMenu(hWnd);
	        HMENU hViewMenu = GetSubMenu(hMenuBar, 2);
	        CheckMenuItem(hViewMenu, ID_TOGGLE_SHOW_FPS, MF_BYCOMMAND | (showFpsOverlay ? MF_CHECKED : MF_UNCHECKED));

	        InvalidateRect(hWnd, NULL, FALSE);
	    }
	    else if (LOWORD(wParam) == ID_TOGGLE_SIM_LOG || LOWORD(wParam) == ID_TOGGLE_FPS_LOG)
	    {
	        // View > Simulation Log / FPS Log -- same independent toggle
	        // pattern as Show FPS above. Only reachable in Simulation (the
	        // items are greyed out otherwise -- see the mode-switch handler),
	        // but guarded here too in case of a stray WM_COMMAND.
	        if (appMode == APP_MODE_SIMULATION)
	        {
	            BOOL isSimLog = (LOWORD(wParam) == ID_TOGGLE_SIM_LOG);
	            BOOL* flag = isSimLog ? &simPhysicsLogEnabled : &simFpsLogEnabled;
	            *flag = !*flag;

	            HMENU hMenuBar = GetMenu(hWnd);
	            HMENU hViewMenu = GetSubMenu(hMenuBar, 2);
	            CheckMenuItem(hViewMenu, LOWORD(wParam), MF_BYCOMMAND | (*flag ? MF_CHECKED : MF_UNCHECKED));

	            // Plain printf, not SIM_LOG -- the OFF message has to show
	            // even though the flag was just cleared.
	            printf("[LOG] %s %s\n", isSimLog ? "Simulation log" : "FPS log", *flag ? "ON" : "OFF");
	        }
	    }
	    return 0;
	}
    ...
    case WM_DESTROY: KillTimer(hWnd, UI_HOTZONE_TIMER_ID); KillTimer(hWnd, AUTO_GRAVITY_TIMER_ID); KillTimer(hWnd, ROCKY_KNEE_HOLD_TIMER_ID); KillTimer(hWnd, SEMNI_JOINT_HOLD_TIMER_ID); PostQuitMessage(0); return 0;
    }
