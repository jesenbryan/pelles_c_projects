...
// just above the Probe 1 Settle button -- toggled from the View > Show FPS
...
// Vertical space reserved above hProbe1ToggleBtn, at the same top-right
...
// and everything else pinned below it -- it would be jarring for Probe 1
...
// so Probe 2 stopped running before the guard ever released, and the
...
//
// Probe 2 stops once its step size falls below
...
//
// Unlike Probe 2, a candidate here is NOT allowed to "fix" a collision by
...
static BOOL rockySeatProbe(Rocky* r, PointF pivot, float deltaDeg,
                           float* outDanglingClearance, float* outComY)
{
    float baseAngle = r->angle;
    PointF shift = rotateRockyBodyAroundPivot(r, pivot, deltaDeg);
    ...
    if (outComY) *outComY = computeRockyMassCenterWorld(*r).y;
...
static void seatRockyDanglingPoint(Rocky* r)
{
    if (r->legHidden && r->bodyHidden)
        return;
    ...
    rockyToppleContactPoints(r, &pivotContact, &dangling);
    ...
    BOOL bMinus = rockySeatProbe(r, pivot, -0.1f * spanDeg, &cMinus, NULL);
    BOOL bPlus  = rockySeatProbe(r, pivot,  0.1f * spanDeg, &cPlus, NULL);
    int dir = 0;
    ...
    if (!rockySeatProbe(r, pivot, dir * spanDeg, NULL, NULL))
    {
    ...
    for (int i = 0; i < SIMULATION_ROCKY_SEAT_SEARCH_ITERATIONS; i++)
    {
        ...
        if (rockySeatProbe(r, pivot, dir * mid, NULL, NULL)) blockedAt = mid;
        else                                                  safe = mid;
    }

    float seatedClearance = 0.0f, seatedComY = baseComY;
    rockySeatProbe(r, pivot, dir * safe, &seatedClearance, &seatedComY);
    ...
    rotateRockyBodyAroundPivot(r, pivot, dir * safe);
    float lift = 0.0f;
    if (robotCollidesWithEnvironment())
...
static void advanceRockySettle(void)
{
    ...
    // TRUE for this tick only when one of Probe 2's four reversal-guard
    ...
    // tiny, already-six-times-halved step size Probe 2 was actually
    ...
    if (!r->legHidden && !rockyKneeSettleSuppressed && !legRestingOnImmovableJoint && rockyProbe1Enabled && rockyKneeSettleStep >= SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
    {
        ...
        {
            SIM_LOG("[SETTLE] kneeAngle=%.2f both directions improve but by different criteria (drop=%.5f/%.5f gain=%.5f/%.5f) -- shrinking instead of picking one, to avoid rocking back and forth\n",
        ...
        else if (bestDrop > SIMULATION_LEG_SETTLE_MIN_DROP)
        {
            ...
            SIM_LOG("[SETTLE] kneeAngle=%.2f bent the knee %.4f deg, body fell %.5f further this tick (of %.5f available)\n",
        ...
        else if (bestClearanceGain > SIMULATION_LEG_SETTLE_MIN_CLEARANCE_GAIN)
        {
            ...
            probe1Committed = TRUE;
            probe1DirectCommit = TRUE;
            SIM_LOG("[SETTLE] kneeAngle=%.2f swung the dangling foot %.4f deg closer to the ground (clearance improved %.5f, body stayed put)\n",
        ...
        else
        {
            ...
            if (scoutWinner != -1)
            {
                ...
                SIM_LOG("[SETTLE] kneeAngle=%.2f scouted real improvement %.1f deg out (drop=%.5f clearanceGain=%.5f), fell %.5f this tick -- nudging that way instead of shrinking\n",
    ...
    else if (!r->legHidden)
    {
        ...
        SIM_LOG("[SETTLE] Probe 1 (knee) skipped this tick -- suppressed=%d legRestingOnImmovableJoint=%d(kind=%d clearance=%.5f) probe1Enabled=%d kneeStep=%.5f (min=%.5f)\n",
    ...
    if (rockyBodySettleStep >= SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
    {
        ...
        // possible, and without the guard Probe 2 just alternates pivots
        ...
        float reversalGuardComX = computeRockyMassCenterWorld(*r).x;
        ...
        if (!reversalGuardArmed && !(r->legHidden && r->bodyHidden))
        {
            RockyLegContact supportA, supportB;
            rockyToppleContactPoints(r, &supportA, &supportB);
        ...
        // same baseBodyAngle pose the pivot itself was just picked from.
        {
            ...
            else if (rockyPivotSide != 0 && currentPivotSide != rockyPivotSide)
            {
                SIM_LOG("[SETTLE] bodyAngle=%.2f pivot flipped sides of the center of mass (side=%d -> %d), but the center of mass is %.4f away and not between two contacts -- a real topple, not vibration, so not halving and not reversal-guarding\n",
                       r->angle, rockyPivotSide, currentPivotSide, reversalGuardComOffset);
        ...
        && !rockyBodyReversalGuardBlockedThisTick)
    {
        ...
        // SIMULATION_LEG_SETTLE_SCOUT_STEP_DEG Probe 2's own scout
        ...
        if (simPhysicsLogEnabled && !(r->bodyHidden && r->legHidden))
        {
            PointF sanityPivot = rockyTopplePivotWorld(r);
            float sanityBaseAngle = r->angle;
            float sanityBaseComY = computeRockyMassCenterWorld(*r).y;
            ...
            for (int sdir = -1; sdir <= 1; sdir += 2, si++)
            {
                ...
                PointF sanityShift = rotateRockyBodyAroundPivot(r, sanityPivot, sanityDelta);
        ...
        SIM_LOG("[SETTLE] converged -- kneeAngle=%.2f bodyAngle=%.2f, no further improvement found\n",
             ...
             && (r->legHidden || rockyKneeSettleSuppressed || legRestingOnImmovableJoint || rockyKneeSettleStep < SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
             && rockyBodySettleStep < SIMULATION_LEG_SETTLE_MIN_STEP_DEG)
    {
...
// Revives Probe 1/Probe 2's own step sizes (and the growth-streak/both-
...
// Probe 2's own direction/identity memory (rockyPivotSide,
...
// number canvasRenderFrame draws, top-right, above the Probe 1 Settle
...
void canvasRenderFrame(float dimAmount)
{
    ...
    // directly above the Probe 1 Settle button (hProbe1ToggleBtn, WM_CREATE/
    ...
    // edge (glWindowWidth - PROBE1_TOGGLE_MARGIN_PX - PROBE1_TOGGLE_WIDTH_PX)
    ...
    if (showFpsOverlay)
    {
        ...
        glRasterPos2i(glWindowWidth - PROBE1_TOGGLE_MARGIN_PX - PROBE1_TOGGLE_WIDTH_PX, glWindowHeight - 24);
        glPushAttrib(GL_LIST_BIT);
        glListBase(fontBase - 32);
...
LRESULT CALLBACK WndProcGL(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY: KillTimer(hWnd, UI_HOTZONE_TIMER_ID); KillTimer(hWnd, AUTO_GRAVITY_TIMER_ID); KillTimer(hWnd, ROCKY_KNEE_HOLD_TIMER_ID); PostQuitMessage(0); return 0;
    }
