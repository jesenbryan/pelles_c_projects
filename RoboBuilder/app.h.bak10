...
typedef struct {
    ...
    // (their ratio, weighted against the body-rect vs leg centroids) --
    // they are NOT the number written to Rob.txt's own weight field.
    // actualWeight below is a separate, real total-mass value (see
    // input.c's hActualWeightEdit) that IS what gets written there, so
    // repositioning the mass-center dot for a quick "what if" never
    // silently changes the exported mass.
    float bodyWeight;
    float legWeight;

    // The real total mass written as the "(gewicht)" value on Rob.txt's
...
typedef struct {
    ...
    // written out as Rob.txt's own "(gewicht)" weight value (see Rocky's
    // own actualWeight field and save.c's saveRockyAsRobArm). Same
    // "harmlessly inert for other robot kinds" convention as the Body Wt/
    // Leg Wt boxes above.
    HWND hActualWeightLabel;
    HWND hActualWeightEdit;
