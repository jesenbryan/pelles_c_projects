...
int fitCircle3(Point A, Point B, Point C, Circle* out)
{
    float a1 = B.x - A.x;
    float b1 = B.y - A.y;
    float a2 = C.x - A.x;
    ...
    float d1 = a1 * (A.x + B.x) + b1 * (A.y + B.y);
    float d2 = a2 * (A.x + C.x) + b2 * (A.y + C.y);

    float det = 2.0f * (a1 * b2 - b1 * a2);

    if (fabsf(det) < 1e-6f)
        return 0;

    out->cx = (b2 * d1 - b1 * d2) / det;
    out->cy = (a1 * d2 - a2 * d1) / det;

    float dx = A.x - out->cx;
    float dy = A.y - out->cy;
    out->r = sqrtf(dx*dx + dy*dy);

    return 1;
}

float pointError(Point p, Circle c)
{
    float dx = p.x - c.cx;
    float dy = p.y - c.cy;

    float dist = sqrtf(dx*dx + dy*dy);
    return fabsf(dist - c.r);
}

float localArcScore(Point* p, int i, int window, Circle* outCircle)
{
    Point A = p[i];
    Point B = p[i + window / 2];
    Point C = p[i + window];

    Circle c;
    if (!fitCircle3(A, B, C, &c))
        return 9999.0f; // bad fit

    float errorSum = 0.0f;

    for (int k = i; k <= i + window; k++)
        errorSum += pointError(p[k], c);

    *outCircle = c;

    return errorSum / window;
}

#define MAX_POINTS 10000

float score[MAX_POINTS];
Circle localCircle[MAX_POINTS];

void computeLocalScores(Point* path, int n, int window)
{
    for (int i = 0; i < n - window; i++)
    {
        score[i] = localArcScore(path, i, window, &localCircle[i]);
    }
}

#define ARC_THRESHOLD 1.5f

int isArcPoint(int i)
{
    return score[i] < ARC_THRESHOLD;
}
...
int buildSegments(Point* path, int n, ArcSegment* out)
{
    ...
    while (i < n - 1)
    {
        ...
        Circle best = localCircle[i];

        while (i < n - 1 && isArcPoint(i))
        {
            i++;
        }
        ...
        out[segCount].start = start;
        out[segCount].end = end;
        out[segCount].circle = best;
